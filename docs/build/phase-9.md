...
> `T11g-2` split again, because its "one tool over `verification/traceability.yaml`" assumed a file
> that does not exist, alongside `verification/coverage.yaml` and the whole `R-<doc>.<section>-<n>`
> requirement ID space 09 §8 is written in. `2a` built the link that _was_ a session's work:
...
**The required set: 91.** `dev/tests/phase-ratchet-is-asserted.py --phase 9` derives it as 90 (09
§10, every row ≤ 9, each suite expanded against §6 and filtered by its member's own due date) ∪ 91
(this table). Exactly one ID comes from the table alone: **V-GAT-002**, which Accept (b) binds and
which no §10 suite name reaches at phase 9. **The table no longer under-names the ratchet**:
...
| ~~**P9-T11g-2**~~ | **Split at IMPLEMENT on 2026-07-31 into `-2a` and `-2b`, because the row's "one tool over `verification/traceability.yaml`" assumed two artifacts that do not exist.** Neither `verification/traceability.yaml` nor `verification/coverage.yaml` is in the tree, and 09 §8's `R-<doc>.<section>-<n>` requirement ID space has never been minted — repo-wide, the only two occurrences of `R-<doc>.<section>-<n>` are the two sentences in 09 §8 that _define_ it. V-MET-002/008/009 all read a requirement→check mapping over ~538 normative statements; V-MET-001 reads a check→implementation one. Two different artifacts, and only the second is a session's work.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | 09 §8, §8.1               | —                                                                                                                                                    | → `T11g-2a`, `T11g-2b`                                                                             | —      |
...
| **P9-T11g-2b**    | **The coverage family — V-MET-002, V-MET-008, V-MET-009 — and the two artifacts 09 §8 assumes.** Mint the `R-<doc>.<section>-<n>` requirement IDs over 01–08's normative statements (~538 by the §8.1 shape), write the curated requirement→check mapping as `verification/traceability.yaml`, and record §8.1's baseline in `verification/coverage.yaml`. Then: **V-MET-002** (every requirement owned by the load-bearing suites — V-CTN/V-BRK/V-REV/V-ISO/V-ADV — maps to ≥1 check, and an unmapped one fails the build), **V-MET-008** (coverage elsewhere may not fall below the baseline), **V-MET-009** (the uncovered list is _published_ on every full run, not merely counted). All three BLOCKING-ALWAYS. `T11g-2a`'s registry is the check→implementation half and is a **prerequisite**, not a substitute: §8's chain is requirement → check → implementation, and only its last link exists today.                                                                                                                                                                                                                                                                                                     | 09 §8, §8.1, §9.4         | `verification/traceability.yaml` · `verification/coverage.yaml` · new `dev/tests/` tool · `dev/L0-CHAIN.txt`                                         | V-MET-002, V-MET-008, V-MET-009                                                                    | large  |
...
does not exist. Neither does `verification/coverage.yaml`, which 09 §8.1 says holds the coverage
baseline. Neither does the `R-<doc>.<section>-<n>` requirement ID space the whole of §8 is written
in — a repo-wide search returns exactly two hits, and both are the sentences in 09 §8 that _define_
the scheme.

So §8's chain is **requirement → check → implementation**, and none of its three links had an
artifact. V-MET-002/008/009 all read the first link: they need ~538 normative statements enumerated
and mapped, which is a unit of its own (`T11g-2b`). V-MET-001 reads the second, and that is this
unit. The split is recorded in the task table above, and `T11g-2b` names the registry built here as
a prerequisite rather than a substitute.
...
**`harness-run`, unit `P9-T11g-2b`** — V-MET-002, V-MET-008, V-MET-009, which need the
`R-<doc>.<section>-<n>` requirement enumeration, `verification/traceability.yaml` and
`verification/coverage.yaml`, none of which exist. Then `T11g-3` (V-CMP-020 and the L0 arms of
V-CTN-004/017), `T11g-4` (the L2 containment arms on `gke-scratch-kube-agents-dev`, including a
runner for the VAP corpora that nothing runs today), then `P9-T11d`.
