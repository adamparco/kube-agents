...
| **P9-T11g-2b-ii**   | **The numerator, and the two gates that read it.** Populate the curated `checks:` mapping in `verification/requirements.yaml` — 09 §8's _"each check declares the requirement IDs it satisfies"_ — then build **V-MET-002** (every requirement **owned by** V-CTN/V-BRK/V-REV/V-ISO/V-ADV maps to ≥1 check; an unmapped one fails the build) and **V-MET-008** (coverage elsewhere may not fall below the baseline). Both BLOCKING-ALWAYS. **Ownership must be derived, not asserted**: 09 §6 already binds each check to a suite and each check's `Source` cell to a spec section, so section→suite ownership falls out of the catalog rather than being minted by the unit that benefits from a smaller obligation. **The ratchet runs against `totals`, not §8.1's published numbers** — §8.1 is reproducible for 02 (148 published, 147 by these rules) and is not uniformly reproducible (78 for 05 against ~204; "96 (groups)" for 06 against ~374), so that audit grouped some documents and enumerated others. A ratchet needs a baseline in the units it measures in; both are recorded in `coverage.yaml` for that reason.                                                                                                                                                                                                                                                             | 09 §8, §8.1               | `verification/requirements.yaml` · new `dev/tests/` tool · `dev/L0-CHAIN.txt`                                                                                                                                    | V-MET-002, V-MET-008                                                                               | large  |
...
**`harness-run`, unit `P9-T11g-2b-ii`** — populate the curated `checks:` mapping and build
**V-MET-002** and **V-MET-008**, both BLOCKING-ALWAYS. Two constraints inherited from this unit and
worth not rediscovering:

1. **Ownership must be derived, not asserted.** V-MET-002 is scoped to requirements _owned by_
   V-CTN/V-BRK/V-REV/V-ISO/V-ADV, and whoever gets to declare ownership can make the gate green by
   declaring everything unowned. 09 §6 already binds each check to a suite and each check's
   `Source` cell to a spec section, so section → suite ownership falls out of the existing catalog.
2. **The ratchet runs against `totals`, not §8.1's published numbers** — see the calibration
   section above for why the two are different populations.
