...
| **P9-T11g-2b-ii-2c-ii**   | **The sixteen, and the green.** Grow the 09 §6 catalog with the rows that close them — each a real, provable property with a suite, a Source, a level and a phase, never a row written to be cited. The sixteen: `R-03.4.3-8`, `R-03.4.3-9` (ancestor `Agent` CR, live tier template), `R-04.5.1-9`…`-16` (04 §5.1's **settle-window** table — a per-kind doc-drift lint over `verify/predicate.go`'s window constants is the honest shape), `R-05.1.2-2` (snapshot `Secret` digests), `R-06.2.3-6` (the _absence_ of a developer-team actor GSA), `R-06.4.2-17` (`ChangePolicy` `fieldPaths` dialect admission), `R-06.4.2-44`/`-45` (the live-Secret digest comparison method, and that digests are never journaled), `R-07.5-4` (authority-never-precedes-machinery). **`R-07.5-4` is the cheap one and the instructive one:** its gate already exists in the tree as `invariants-gate.py::check_write_verbs_have_machinery`, and it is unmapped only because **no catalog ID claims it** — it closes with a new 09 §6 row pointed at that function, not with a new implementation. Then map all sixteen, turn V-MET-002 green, **move its live arm onto `dev/L0-CHAIN.txt`**, add its `implementations.yaml` row, and **retire `check_phase_gate_publishes_the_coverage_remainder` in the same commit that moves the line** — so the two never both fail to cover it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 09 §6, §8, §8.1           | `docs/design/09-verification-and-validation.md` · `verification/requirements.yaml` · `dev/L0-CHAIN.txt` · `verification/implementations.yaml` · `dev/tests/invariants-gate.py`                                                        | V-MET-002, V-MET-008 (regression)                                                                  | large  |
