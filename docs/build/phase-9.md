...
| **P9-T11g-2b**    | **The coverage family — V-MET-002, V-MET-008, V-MET-009 — over a requirement enumeration that does not exist yet.** Mint the `R-<doc>.<section>-<n>` requirement IDs over 01–08's normative statements (~538 by the §8.1 shape) as a **new** artifact, and record §8.1's baseline in `verification/coverage.yaml`. **Not into `verification/traceability.yaml`**, which already exists and is V-MET-011's: it maps the 177 Verification _bullets_, a different and much smaller population, and overloading one file with two ID spaces would give V-MET-011's green a second thing to mean. Then: **V-MET-002** (every requirement owned by the load-bearing suites — V-CTN/V-BRK/V-REV/V-ISO/V-ADV — maps to ≥1 check, and an unmapped one fails the build), **V-MET-008** (coverage elsewhere may not fall below the baseline), **V-MET-009** (the uncovered list is _published_ on every full run, not merely counted). All three BLOCKING-ALWAYS. §8's chain is requirement → check → implementation: `T11g-2a` built the last link, `traceability.yaml` covers a slice of the first, and this unit builds the rest of it.                                                                                                                                                                | 09 §8, §8.1, §9.4         | new `verification/requirements.yaml` · `verification/coverage.yaml` · new `dev/tests/` tool · `dev/L0-CHAIN.txt`                                     | V-MET-002, V-MET-008, V-MET-009                                                                    | large  |
...
`R-<doc>.<section>-<n>` requirement enumeration over ~538 normative statements and
`verification/coverage.yaml`, neither of which exists. **Not** an extension of
`verification/traceability.yaml`: that file is V-MET-011's 177-bullet mapping, and putting a second
ID space in it would give V-MET-011's green a second thing to mean. Then `T11g-3` (V-CMP-020 and the L0 arms of
V-CTN-004/017), `T11g-4` (the L2 containment arms on `gke-scratch-kube-agents-dev`, including a
runner for the VAP corpora that nothing runs today), then `P9-T11d`.
