...
| P1-T8  | `(tier,scope)` cardinality webhook + tier immutability       | 06 §1.2, §10           | `platformagent_webhook.go` (per-(tier,scope) `scopeIdentity`; ValidateUpdate tier-immutable; `validateClosedAllowlist`) + `platformagent_types.go`/CRD (2 CEL non-empty-allowlist rules)                                                                       | high         | duplicate (tier,scope) CR rejected; changing `tier` rejected; different tiers coexist; enabled chat + empty allowlist rejected                                             | ✅     |
| P1-T9  | **Cloud GSA → viewer-only IAM** (the real cloud-write delta) | 07 Phase 1             | `provision_04_gcp_iam.sh` (retire `gke_admin_roles`; read-only default+fallback; active privileged-role reconcile), `common.sh` (`PLATFORM_AGENT_PERMISSION_SET` default→read-only, `gke-admin` coerced), docs                                                 | load-bearing | GSA has no `container.clusterAdmin/admin`, `monitoring.admin`; role-selection emits no admin for any input; `gcloud ... get-iam-policy` viewer-only (scratch GKE, pending) | ✅     |
