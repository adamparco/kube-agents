...
(`dev/verify/kind-calico.yaml` + `dev/tests/egress-enforcement.sh`) that applies a tier's
...
| P5-T6  | E-A — Egress **enforcement** proof on Calico (off-allowlist DENIED / on-allowlist ALLOWED)            | E       | 03 §11; 05                   | `dev/verify/kind-calico.yaml`, `dev/tests/egress-enforcement.sh`                                                                                                         | On a Calico cluster: default-deny + allowlist applied → off-allowlist egress **blocked**, on-allowlist **allowed**; real-deny-vs-setup-error distinguished; no Calico ⇒ scratch-GKE, **deferred-not-faked**; L7 proxy noted deferred     | ✅     |
...
| P5-T10 | Phase 5 verification: `verify-phase5.sh` (Kind gate, Acc a–d) + Calico egress regress + regress prior | R+E+H+T | 07 §5; 06 §10; 03 §11; 08 §7 | `dev/verify/verify-phase5.sh` (new), reuse `egress-enforcement.sh`/`negative-attenuation.sh`/`verify-phase{2,3,4}.sh`, scorer + go tests                                 | `verify-phase5.sh` exit 0: scorer blocks unmitigated-high (a), all-tier netpol shape + Calico deny (b), goldens+VAP dry-run reject un-hardened pod (c), PR trailer + audit tie (d); **03 §11 + verify-phase{2,3,4} not regressed**       | ✅     |
