...
| P3-T13 | Phase 3 verification: `verify-phase3.sh` (A6) + router go-test matrix (B6) + regress          | A+B   | 07 §5; 03 §11; 06 §10                | `local-dev/kind/verify-phase3.sh` (new), reuse `local-dev/tests/negative-attenuation.sh`, router tests                                                                                                                   | `verify-phase3.sh` exits 0 (placement, SAR isolation, VAP admit/deny, image, Calico egress + ExternalName reachability, cardinality dup + tier immutability); `go test ./...` green; **03 §11 + verify-phase2 not regressed**                                         | ✅     |
