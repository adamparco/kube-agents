...
**A6 — Kind gate.** `dev/verify/verify-phase3.sh` mirroring `verify-phase2.sh`, reusing
...
| ------ | --------------------------------------------------------------------------------------------- | ----- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| P3-T1  | Wire the developer-team baked image into `defaultImageForTier` (A3.1) + update tests          | A     | 07 §2(a); 06 §3                      | `k8s-operator/internal/controller/manifest_helpers.go`, `manifest_helpers_test.go`                                                                                                                                       | `go test ./internal/controller/...` green; `TestDefaultImageForTier(developer-team)==defaultDeveloperTeamAgentImage`; tier-aware image test covers dev-team + override                                                                                                | ✅     |
| P3-T2  | Fix `otelTelemetryEnvVars` tier label `"platform"` → `EffectiveTier(agent)` (A3.2)            | A     | 05 telemetry attribution             | `k8s-operator/internal/controller/agent_manifests.go` (~:369)                                                                                                                                                            | `go test ./...` green; rendered OTEL attr for a dev-team CR shows `tier=developer-team`; platform/cluster-admin goldens unchanged                                                                                                                                     | ✅     |
| P3-T3  | Add the developer-team placement clause to the admission webhook (A1)                         | A     | 03 §3; invariant 4; 06 §1.2          | `k8s-operator/internal/webhook/agent_webhook.go`, `agent_webhook_test.go`                                                                                                                                                | Unit: dev-team CR with `metadata.namespace != scope.namespace` REJECTED (scope-mismatch); `==` ADMITTED; `go test ./internal/webhook/...` green                                                                                                                       | ✅     |
| P3-T4  | Create `agents/developer-team/` baked persona tree + Dockerfile target (A3.3)                 | A     | 07 §2(a); 03 §11 (read-only persona) | `agents/developer-team/**` (SOUL/AGENTS/config.yaml read-only, submit-suggestion, no propose-child), `deploy/docker/Dockerfile`                                                                                          | `make docker-build-developer-team` builds; config has no `create_cluster`/`apply_manifest`; no propose-child cascade                                                                                                                                                  | ✅     |
...
| P3-T12 | B5 — Audit/attribution surface (`Tier`/`Clarify`/`ThreadID`/`ModeSticky`) + Replier seam      | B     | 06 §2b/§10                           | `k8s-operator/internal/router/{audit,gateway}.go` + tests                                                                                                                                                                | Sticky turn records `Mode==ModeSticky`+Tier+Identity; NL clarify records `Clarify==true`, `Mode==ModeInference`, `dispatched==false`, fake Replier got a question naming candidates; delivered turn records Tier/Identity/ThreadID                                    | ✅     |
| P3-T13 | Phase 3 verification: `verify-phase3.sh` (A6) + router go-test matrix (B6) + regress          | A+B   | 07 §5; 03 §11; 06 §10                | `dev/verify/verify-phase3.sh` (new), reuse `dev/tests/negative-attenuation.sh`, router tests                                                                                                                             | `verify-phase3.sh` exits 0 (placement, SAR isolation, VAP admit/deny, image, Calico egress + ExternalName reachability, cardinality dup + tier immutability); `go test ./...` green; **03 §11 + verify-phase2 not regressed**                                         | ✅     |
| P3-T14 | Docs (INSTALL Phase 3 section, LEDGER, memory) + open PR → main on fork; auto-merge           | A+B   | roadmap; AGENTS.md                   | `INSTALL.md`, `docs/build/LEDGER.md`, memory                                                                                                                                                                             | PR opened on fork base `main`; all CI green + `mergeStateStatus: CLEAN`; no HALT; PR URL shared                                                                                                                                                                       | ✅     |
