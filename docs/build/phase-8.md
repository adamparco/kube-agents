...
`dev/tests/install-path-wired.py` now implements it generically (every numbered step is invoked
...
- The `ExternalName` aliases a developer-team agent needs for in-namespace `litellm` /
  `github-token-minter` DNS are asserted by `dev/verify/verify-phase3.sh:151-154` but not
...
- **LSN-015 (closed here).** The first real multi-tier install deadlocked because the
  ...
  `dev/verify/multi-agent-namespace-l2.sh` now runs two, and the lesson recurred one level up
...
**Resolved in P8-T5 (2026-07-25).** The finding named one half of the defect. Taking a census of
every `(image, tag)` pair in the tree against every pair the workflows produce turned up the other:
**`:v0.1.0` was published for no image at all.** CI produced `:latest` and `:${sha}` and nothing
else, so `platform-agent:v0.1.0` — an image whose _name_ was published, pinned by every manifest in
the tree — was as unpullable as the three that had no build step. Neither half is visible from
inside the file that is wrong: a manifest cannot see the workflow, and the workflow cannot see who
pins what. The defect lives in the join, so the check has to be the join.

The fix was constrained by something already written down. Six comments across the tree promised
"pinned to an immutable version tag (not `:latest`)". Publishing `:v0.1.0` from `main` — the obvious
repair — would have made the tag resolvable but _moving_, falsifying all six and being arguably
worse than `:latest` because it still looks pinned. So: `main` → `:${sha}`, a `v*` git tag → the
release tag, nothing else, and the publish workflows **refuse** a tag build whose git tag disagrees
with `KAGE_IMAGE_VERSION` in `tags.env`. That is the only scheme under which the six existing claims
become true. `tags.env` is now the single source of truth for both pins and parses as shell _and_ as
make.

Scope beyond the finding, per the unit row: Cloud Build now builds the operator and router too (the
config generalized over `_CONTEXT`/`_DOCKERFILE`/optional `_TARGET`); the Apple-silicon
`PREBUILT_BINARY` hatch is documented along with why the agent tiers cannot have one; `:latest` is
gone from every build and push path. Removing `:latest` meant repointing its three consumers
(`provision_08`, `provision_11`, `provision_12` defaults) — leaving them would have created a fresh
instance of the very defect being closed.

**V-CMP-002 green at L0** (`dev/test_image_provenance.py`, 10 tests, **14/14 mutations
caught**). M10 — retagging the router's kustomize `newTag: v0.1.0` to `latest` — passed silently on
the first battery. A real gap, not a fussy one: the lookahead resolved the CR form (`image:`/`tag:`)
but not kustomize's (`newName:`/`newTag:`), so the router's _actual deployed tag_ was the one pair
the check could not see. Fixed, plus a vacuity assertion pinning the kustomize form so it cannot
...
**Resolved in P8-T6 (2026-07-25).** `dev/tests/invariants-gate.py` runs eight checks and is
wired into `dev/L0-CHAIN.txt`, which a new `l0-checks.yml` workflow executes on every PR — so
the chain itself is now the single definition site for "what runs pre-merge" (V-MET-013) instead of
a list retyped in three places. Six rules moved from prose to a build failure: invariants 7, 8, 12,
13, the destructive-test guard (LSN-005) and the `make`-target context discipline (LSN-018).

The finding scoped the unit to checks 7 and 8. Three things it did not know about turned up:

1. **Most closed lessons were not closed.** The last check written,
   `check_closed_lessons_are_executable`, asserts LSN-019 directly: a lesson's Mechanization field
   must name a file that exists and that something automatic actually runs. It **reopened 13 of 17**
   on its first run, every one of them closed against a check ID, a `binding.md` clause or a spec
   section. Eight were re-cited to artifacts that genuinely run; five had nothing behind them and
   are honestly open. **The open count went 4 → 6, past the threshold of 5** — so the next
   invocation is an improvement pass and nothing else (`binding.md` §Thresholds). That is the gate
   working, not a regression: lowering the count by re-closing a lesson against a spec section would
   restore yesterday's number and none of the enforcement.
2. **`make install KUBECTL="kubectl --context …"` was accepted and silently ignored.** The recipes
   piped into a bare `kubectl`, so the CRD went to `kubectl config current-context` — which on this
   host may be the live GKE cluster. It was the right cluster by luck. Now `KUBE_CONTEXT=` is the
   parameter, `KUBECTL=` is a hard `$(error)`, and a new `ctx-guard` prerequisite on all ten
   Deployment targets refuses an unanchored ambient context while always printing the cluster it
   resolved to. LSN-018.
3. **A local `actionlint → exit 0` is not the same run as CI's.** actionlint delegates every `run:`
   block to shellcheck and says nothing when shellcheck is absent; P8-T5 recorded a local green and
   went red in CI on four SC2140 findings. `dev/toolchain-preflight.sh` makes the asymmetry an
   error in both directions and runs as a step of `actionlint.yml`, so a runner image that drops
   shellcheck fails loudly instead of quietly linting less. LSN-020.

Deliberately not mechanized: invariants 1–6, 9, 10 and 11 are judgements about intent, and a script
that pretended to check them would be worse than the checklist. Each mechanized rule is now marked
`[gated: <function>]` in `invariants.md` so the boundary is legible from the checklist side too.
Every check carries a VACUOUS guard and exits **2** (distinct from pass and from fail) when its
corpus is missing — V-MET-014, a check that cannot fail is not evidence. **10/10 mutations caught.**

The tenth was added because the gate caught itself. `_deferral_rows()` filtered the Deferrals table
with `len(cells) >= 5`, so a row that omitted the owner column _entirely_ — rather than leaving it
blank — was dropped before the check looked at it, and the tree held a live instance from P8-T4. The
original mutation test _blanked_ a cell and kept five columns; blanking and deleting had different
consequences and only one was tested. Short rows are now padded rather than filtered, which makes the
two indistinguishable to the check.
...
next drift to be found by the next reader, so the deliverable is `dev/tests/docs-truth.py` —
...
| **P8-T5** ☑ | Image provenance. **Done:** all seven first-party images now publish, mirror to GAR, sign, and build on PRs — `kage-router`, `cluster-admin-agent` and `developer-team-agent` were in none of those four sets. `:v0.1.0` is made real by an immutable scheme (`main` → `:${sha}`; `v*` git tag → the release tag; the tag build **fails** if the git tag disagrees with `KAGE_IMAGE_VERSION` in `tags.env`), which is the only repair that does not falsify the six "immutable version tag" comments already in the tree. `:latest` removed from every build/push path and its three provisioning consumers repointed. Cloud Build generalized to cover operator and router; `PREBUILT_BINARY` documented, with why the agent tiers cannot use it. **Found en route:** `:v0.1.0` — pinned by every manifest — was published for **no image at all**; the finding only knew about the three missing names. **V-CMP-002 green (L0, 10 tests, 14/14 mutations).**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 05 §3               | `tags.env` · `.github/workflows/docker-publish-{ghcr,k8s-operator,gcp}.yml` · `.github/workflows/docker-build.yml` · `deploy/docker/cloudbuild.yaml` · `Makefile` · `provision_0{8},1{1,2}` · `dev/test_image_provenance.py` · `docs/site/.../deploy/docker-images.md`                                                                                 | **V-CMP-002**                                            | high             |
| **P8-T6** ☑ | Make the invariants gate **mechanical**: a pre-merge script that fails any diff adding a write verb to an agent identity while broker/classifier/journal/undo are absent, and that flags a net reduction in security assertions. **Done:** `invariants-gate.py`, **8 checks / 8 pass / 9-of-9 mutations caught**, wired into a new `L0-CHAIN.txt` that `l0-checks.yml` runs on every PR — one definition site for "what runs pre-merge". Gates invariants **7**, **8**, **12**, **13** plus the destructive-test guard (LSN-005) and `make`-target context discipline (LSN-018); 1–6, 9, 10, 11 stay judgements and are marked `[gated: …]` or not in `invariants.md`. Exit **2** = the gate could not run, distinct from pass and fail. **Found en route:** `check_closed_lessons_are_executable` **reopened 13 of 17 lessons** — open count **4 → 6, past the threshold of 5**, so an improvement pass comes next; `make install KUBECTL=…` was accepted and silently ignored (**LSN-018**, now `KUBE_CONTEXT=` + `ctx-guard`); and **LSN-020** closed by `toolchain-preflight.sh` running inside `actionlint.yml`.                                                                                                                                                                                                                                                                                                                                                                                                                               | README, 07 §5       | new `dev/tests/invariants-gate.py` (or `.sh`) · `.claude/harness/invariants.md` · CI wiring                                                                                                                                                                                                                                                            | **V-MET-003**, **V-MET-004**, V-MET-006, V-MET-007       | **load-bearing** |
...
| **P8-T8**   | Consolidated gate `dev/verify/verify-phase8.sh` + a live-target checklist; regression `verify-phase7.sh`. **T8a ☑** every script in the transitive L2 closure carries a PRECONDITIONS block declaring P1/P3/P6 as real-or-waived-with-an-argument; `verify-phase2/3` and `chaos-suite` source `lib/preconditions.sh` and call it, replacing the `apply`-then-`sleep 6` LSN-002 pattern. **T8b ☑** the lint widened from the 6 chain lines to the transitive closure ∩ verdict-rendering scripts = **15**, ratcheted by `L2_SCOPE_FLOOR`. **T8c** — `verify-phase8.sh` + the live-target checklist — still open. **Four defects found by running the machinery rather than reading it**, each now carrying a live negative control: P1 was green over a **6 h 25 m stale** operator (digest without freshness); P1 read `.items[0]`, often the **Terminating** pod of the previous revision; P3 polled for an absence that a controller-owned object never reaches; and closing a deferral had become an exit from being checked. Two more the Calico target exposed: the `.status.hard` race in `tenant-isolation-l2.sh`, and `L2-CHAIN.txt`'s single-target run loop silently not running 2 of its 6 lines. A seventh came from running a **green** script six times rather than once: `verify-phase3.sh` was failing **2 in 3** on empty pod reads, because a label selector still matches the generation `p3_force_recreate` just deleted — fixed by `p3_pod_of_deploy`, with `verify-phase2.sh`'s identical block fixed alongside it (LSN-025). | 07 §5               | `dev/lib/preconditions.sh` · `dev/tests/invariants-gate.py` · `dev/L2-CHAIN.txt` · the 9 transitive L2 scripts · new `dev/verify/verify-phase8.sh` (T8c)                                                                                                                                                                                               | all of the above + **V-MET-007**                         | **load-bearing** |
| **P8-T9**   | Finish **V-CTR-002**: implement the three unimplemented 06 §1.2 rules — **V-6** cross-object ceiling (child RBAC ⊆ parent), **V-8** budget clamp, **V-10** reader-only ServiceAccount override — and build the full **V-1…V-10** L2 negative-test suite, one rejected fixture per rule, each denial asserted to name its field path.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 06 §1.2, 08 §5      | `internal/webhook/agent_webhook.go` · `api/v1alpha1/*_types.go` (V-8 bounds) · new `dev/verify/webhook-negatives-l2.sh` · CRD + goldens                                                                                                                                                                                                                | **V-CTR-002** (completes it)                             | **load-bearing** |
| **P8-T10**  | Implement **V-MET-011** — the traceability matrix. Every bullet in the six **Verification** sections of specs 01–08 maps to at least one check ID in 09, and a bullet with no check ID fails the lint. Curated, not fuzzy-matched: the artifact is a checked-in `verification/traceability.yaml` keyed by `<spec>§<section>#<n>` with the bullet text and its check IDs, and the lint asserts the file and the specs agree in **both** directions — a new bullet with no entry fails, an entry naming a check ID that 09 does not define fails, and a stale entry whose bullet text no longer matches fails. 176 bullets.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 09 §14              | new `verification/traceability.yaml` · `dev/tests/spec-ids.py` (extend) · L0-CHAIN                                                                                                                                                                                                                                                                     | **V-MET-011**                                            | **load-bearing** |
...
**P8-T10 must land before MILESTONE too, and it is not a deferral.** V-MET-011 is `L0` and V-MET is
BLOCKING-ALWAYS, so 09 §9.6 forbids deferring it — the ledger's V-MET-006 lint refused the deferral row
when this pass tried to write one, which is the check working. "The generator is not written" is
unwritten work, not an external blocker, so it gets a task and a gate instead of a row in the deferrals
table. The work is bounded and the count is known: 176 bullets, each needing at least one check ID.
