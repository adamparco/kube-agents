...
**V-CMP-002 green at L0** (`local-dev/test_image_provenance.py`, 10 tests, **14/14 mutations
...
**Resolved in P8-T6 (2026-07-25).** `local-dev/tests/invariants-gate.py` runs eight checks and is
wired into `local-dev/L0-CHAIN.txt`, which a new `l0-checks.yml` workflow executes on every PR — so
...
3. **A local `actionlint → exit 0` is not the same run as CI's.** actionlint delegates every `run:`
   ...
   went red in CI on four SC2140 findings. `local-dev/toolchain-preflight.sh` makes the asymmetry an
