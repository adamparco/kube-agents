...
- **V-CMP-022** — every trigger class in 04 §4.1 has a deployed delivery path (not a documented
  one). `L2`
- **V-CMP-023** — every operational command in 06 §2b.1 (`pause`, `resume`, `freeze`, `undo`,
  `approve`, `reject`, `status`) is implemented on **both** the chat surface and `kubectl`/API. `L2`
...
| V-GAT-020 | Approval identity is broker-verified on all three surfaces (chat, `kubectl`, API); an approval submitted by the agent's own identity on a human's behalf is rejected and raises a security event ¬                                                                                                                                                                 | 04 §2.3, §3.1      | L2         | 10    |
...
| V-OBS-007 | Routing mode is audited: every chat turn records `routingMode ∈ {slash, handle, nl}` with requester, resolved agent, and a trace ID linking to the envelope ¬                                                                                                                                                                                                      | 02 §2.4            | L1, L2     | 15    |
...
| V-MET-002 | Every normative requirement maps to ≥1 check; unmapped requirements fail the build ¬                                    | L0  |
...
### 12.1 Cross-document conflict register

Distinct from §12's ambiguities: these are places where **two specs state different things**. A
harness cannot verify an implementation against a contradiction — whichever value it asserts, the
other document says it is wrong. Each row carries a proposed resolution so the decision is a review,
not a rediscovery.

**Gate rule:** a check blocked on an unresolved conflict is recorded `deferred` with the conflict ID
as its blocker (§9.6). Because **BLOCKING-ALWAYS checks may not be deferred** (§9.6), an unresolved
conflict blocking one of those halts the build — which is the correct outcome: the specification is
not yet verifiable.

| #    | Conflict                                                                                                                                                          | Proposed resolution                                                                                                                                        | Blocks               |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| X-1  | **Broker port** — `8443` (05 §1.1, 06 §4.1) vs `8643` (08 §2.3)                                                                                                   | `8443` for the broker; `8444` for the mesh on the agent pod. Correct 08                                                                                    | V-BRK-007…010        |
| X-2  | **Broker transport** — gRPC (05 §1.1) vs HTTP+JSON `POST /v1alpha1/actions` (06 §4.1)                                                                             | HTTP+JSON — 06 owns wire format and already specifies the route and schema. Correct 05                                                                     | V-CTR-005            |
| X-3  | **Agent Deployment name** — `<agent>-gateway` (08 §7) vs `<agent>` (05, 06)                                                                                       | `<agent>` for the Service, `<agent>-gateway` for the Deployment; state both explicitly in 08 §2.1                                                          | V-CMP-001, V-RUN-001 |
| X-4  | **Label set** — three labels (05 §8) vs five (08 §2.5: `role`, `agent`, `tier`, `scope`, `parent`)                                                                | Five. 05 §8 is an incomplete restatement                                                                                                                   | V-RUN-004            |
| X-5  | **`ActionRecord` retention** — 30/90/180 d + a 7/30/90 d "guaranteed undo window" (05 §1.2) vs 30/90/365 d, no window (06 §4.3)                                   | Adopt 06's TTLs; add `spec.retention.undoWindowExpiresAt` so the undo window is a field, not prose                                                         | V-REV-008            |
| X-6  | **Snapshot inline threshold** — 256 KiB (05 §1.2) vs 1 MiB (06 §4.3)                                                                                              | 1 MiB, in 06                                                                                                                                               | V-BRK-006            |
| X-7  | **Blast-radius caps** — ≤50 objects / ≤10% (05 §6) vs >100 / >0.5 hard cap (06 §4.2), envelope max 50 operations (06 §4.1)                                        | Reconcile as: per-action gate at 50, hard abort at 100, fraction defined against workload objects in scope                                                 | V-GAT-011, V-NFR-006 |
| X-8  | **Initiative budget shape** — per-class (05 §6: 50 routine + 10 elevated/h) vs class-agnostic CRD fields (06 §1.1)                                                | Per-class. The CRD must gain per-class fields or 05's budget is unexpressible                                                                              | V-PRO-020            |
| X-9  | **Undo CLI** — `kubectl kube-agents undo` (05 §1.3) vs `kubectl kage undo` (06 §2b, §4.1)                                                                         | `kage`. Correct 05                                                                                                                                         | V-REV-002            |
| X-10 | **Undo linkage** — `spec.trigger.undoOf` (05 §1.3) vs `status.undoneBy` on the original (06 §4.3)                                                                 | Carry **both**: forward link on the undo record, reverse on the original. Neither alone is queryable                                                       | V-REV-005            |
| X-11 | **Delegation chain ID** — 05 §1.4 promises "one queryable object graph"; `ActionRecord` has no chain field (06 §4.3)                                              | Add `chainId` as a label on `ActionRecord`, propagated from `MeshRequest.chain`                                                                            | V-MSH-003            |
| X-12 | **Auto-brake on budget/flap** — auto-pause the agent (05 §1.5) vs refuse-and-escalate without pausing (04 §4.2, 06 §4.4)                                          | Escalate without pausing for budget/flap; reserve auto-pause for repeated `forbidden`, failed rollback, and anomaly                                        | V-PRO-015            |
| X-13 | **Audit-policy coverage** — 05 §5.1 filters on `system:serviceaccounts:kubeagents-system`, but developer-team reader **and** actor SAs live in the team namespace | Filter on the `kube-agents/*` SA **name pattern** across all namespaces, not one namespace. As written, SLIs 1–3 are blind to the highest-cardinality tier | **V-OBS-001…003**    |
| X-14 | **Approval TTL** — 24 h (04 §3.1) vs 4 h (06 §4.4)                                                                                                                | One default in 06, referenced by 04                                                                                                                        | V-GAT-007            |
| X-15 | **Production label** — `env=production` or `kube-agents/environment: production` (06 §4.2), unspecified in 03 §5.2                                                | Name one canonical label; accept the second only as a documented alias                                                                                     | V-GAT-010            |
| X-16 | **`/uncontest`** referenced in 06 §4.4 but absent from the command table in 06 §2b.1                                                                              | Add it to the table as roster-only                                                                                                                         | V-PRO-006            |
| X-17 | **Mesh depth** — "depth > 3 rejected" (05 §1.4) vs "`depth >= 4`, default budget 3" (06 §7), with no CRD field for the budget                                     | Numerically consistent; add the field or drop "configurable"                                                                                               | V-MSH-006            |
| X-18 | **Journal durability "100% — enforced, not measured"** (05 §6) — true for Kubernetes admission, **detect-only** for cloud writes (06 §8)                          | Split the requirement: enforced for K8s, reconciled-within-SLO for cloud                                                                                   | V-BRK-003            |

### 12.2 Design gaps with no owner

Named as load-bearing in one or more specs, but no component owns them, so nothing can be verified.

| #    | Gap                                                                                                                                                                                                    | Needed before the check exists                                                                                                                                                                     | Blocks        |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| G-1  | **`action-id` cannot be enforced on deletes or subresources** — a `DELETE` admission request carries no client object, so no annotation can be attached                                                | Restate 03 §4.3 honestly: create/update/patch **rejected at admission**; deletes and cloud calls **detected by reconciliation** within a stated SLO. Give the reconciler an owner and a cycle time | **V-BRK-004** |
| G-2  | **Journal-completeness reconciliation** is cited in 03 §4.3/§7 and 08 §4 but is not a component in 05 §1                                                                                               | Name the component, cadence, and action on mismatch                                                                                                                                                | **V-BRK-003** |
| G-3  | **Anomaly detection and auto-pause** is load-bearing in 03 §6 and §8.1 and cited twice in 08 §4; no component, no thresholds, no baselines                                                             | Assign to a named component; define the three dimensions and their thresholds                                                                                                                      | V-ADV-005     |
| G-4  | **`vap-agent-scope` is asked to do cross-object checks CEL cannot express** (ancestry, tier-template membership)                                                                                       | Split the obligations explicitly between the in-tree VAP and the controller's webhook, and require `failurePolicy: Fail` on the webhook — an outage must not silently open the ancestry check      | V-CTN-012/013 |
| G-5  | **Controller namespace confinement is not RBAC-expressible** — placement namespaces are created dynamically, so the natural implementation is a `ClusterRoleBinding`, which voids the 08 §4 mitigation | Per-namespace `RoleBinding` created at provisioning time, or a VAP restricting the controller to namespaces carrying a placement label                                                             | V-CTN-014     |
| G-6  | **Label↔SA pinning does not defeat a compromised controller**, which sets labels itself                                                                                                                | Tighten 03 §4.3: a pod binding an actor SA must also be transitively owned by the `Agent` CR whose actor SA it binds, **and** run the broker image                                                 | **V-CTN-014** |
| G-7  | **No anti-replay** — a captured, still-credentialed envelope replays; idempotency dedupe is defeated by altering one byte of the key                                                                   | Bind envelopes to a nonce or a short issued-at window; reject repeats of `(traceId, idempotencyKey)`                                                                                               | V-ADV-008     |
| G-8  | **The `kube-system` add-on allowlist** exempted by 03 §3.3 rule 6 is never declared anywhere                                                                                                           | Enumerate it as named objects (not kinds) beside the cluster-admin actor template, as a code constant                                                                                              | V-CTN-022     |
| G-9  | **No code-floor rule for copying Secret material into a non-Secret object**, though 03 §8 requires it be gated                                                                                         | Add a `secret-material-egress` rule with a defined comparison (live Secret value match, not entropy)                                                                                               | V-ADV-007     |
| G-10 | **"Platform may not operate tenant workloads directly"** (03 §3.2) is in no template and no forbidden-set rule                                                                                         | Make it a classification rule (`cross-tier-direct-operation` ⇒ gated) or drop the claim                                                                                                            | V-CTN-021     |
