...
| V-CTR-017 | **The brake's production input source tells the truth about what it could not read**: each of the four reads fails in isolation without blinding the other three, and the 06 §4.4 row reported is the one whose own input failed; a value retained across a failing refresh is still served one second inside `MaxFreezeStaleness` and withdrawn one second past it; the freeze view carries the instant of the **read**, never the instant it is served; a roster that resolved and then vanished is withdrawn at once rather than aged out, so a deleted roster cannot go on approving gated actions until the ceiling catches up; the journal probe is given no staleness tolerance at all; and construction refuses both a cache TTL that could outlive the ceiling and an unnamed budget accountant ¬ | 06 §4.4 | L1     | 9     |
