...
| C15      | **ChatOps gateway & router**                       | **The fleet's single human entrypoint, Slack-first.** Holds the **one** Slack app for the whole fleet and its **single** Socket Mode connection, normalizes each event into one internal message, enforces the target agent's `allowedUsers` **before** dispatch, and routes to the addressed agent over the per-agent transport — `/kage` verb, `@kage <handle>`, thread affinity, channel binding, NL inference last ([02](02-agent-personas.md) §2.4, [06](06-api-and-data-contracts.md) §2b). Google Chat is the fully supported opt-in second ingress, on the **same** normalizer and the **same** dispatch. Also delivers **gate prompts** (Block Kit) and **action reports** back to the humans ([04](04-workflow-model.md) §3). Configured by the cluster-scoped **`ChatOpsConfig`** singleton `default`; runs as **one active replica with a lease-elected standby** — never two. **Credential-bearing:** the Slack app and bot tokens live here and nowhere else (§1.8) | `k8s-operator/cmd/router/` + `internal/router/`; Slack **Socket Mode** (outbound WebSocket, no public ingress) + optional Chat Pub/Sub pull; keyed on `internal/agentindex`                  | Exists (extend — Slack ingress) |
...
### 1.8 The ChatOps gateway & router (`C15`) in detail

**Slack is the reference platform.** Every default, every example, and the provisioning path are
Slack-first. **Google Chat stays fully supported** and is neither deprecated nor second-class in
behaviour — it is opt-in rather than default, and it rides the same normalizer and the same dispatch
path described below. Nothing in this section is Slack-only except the ingress adapter itself.

**Shape.** One `Deployment` `kage-router` and one ClusterIP `Service` in the hub's
`kubeagents-system` namespace (§3): **exactly one _active_ replica, with a lease-based leader
election electing a warm standby** — not a normally-scaled Deployment, and never two active replicas.
That is forced by the single-connection constraint below, not chosen for cost: a second active
replica would open a second Socket Mode connection and **silently split ingress**, so some turns
would land on a router with stale routing state and neither replica would look unhealthy. Only the
lease holder dials Slack; the standby runs, holds no connection, and takes over on lease expiry
(§6 gives the failover budget).

It holds no cluster or cloud write credential: its only Kubernetes authority is
`agents get/list/watch` (to build `internal/agentindex`), `chatopsconfigs get/list/watch`, the
leader-election `Lease`, and `actionrecords/status` approvals (§3). It is **not** a writer; a human
approval delivered through it still terminates in the broker's own re-authorization (F8 step 4).

**Where its configuration and credentials live: the `ChatOpsConfig` singleton.** Fleet-level chat
configuration is a **cluster-scoped `ChatOpsConfig` CR**, a singleton named **`default`**, installed
with the hub and owned by `kubeagents-system` — not a ConfigMap, not router flags, and not per-agent
fields. It carries the Slack app / bot / signing token references (Secrets in `kubeagents-system`),
the Socket Mode settings including `maxConnections`, which is **fixed at 1 as a code constant** and
not an operator dial, `commandName: kage`, and the Block Kit toggle. **Per-agent** CRs carry only the
per-agent bits — the agent's channel bindings and its `allowedUsers`. The split is the same one this
section keeps making: fleet-level identity and transport belong to the one component that terminates
the connection; per-agent addressing and authorization belong to the agent being addressed.
[06](06-api-and-data-contracts.md) §2b owns the schema — reference it, do not restate the fields.

**One Slack app for the whole fleet, held by the router — the load-bearing point.** A Slack **app
token permits exactly one Socket Mode connection**. That single sentence decides the architecture:

- The **previous generation put a Slack relay in each agent pod**, each with its own bot and app
  token. Two agents therefore could not both be connected, which is precisely why every child tier
  shipped `slack.enabled: false` and why only the Platform Agent was ever reachable from chat. That
  was not a configuration oversight; it was the platform constraint surfacing.
- Centralising the connection in `C15` turns the constraint into the **reason the router exists**.
  One app, one token pair, one connection, one place that parses untrusted chat text — and an
  arbitrary number of agents behind it, because ingress and dispatch are separate (below).
- **The per-pod Slack relay is retired.** An agent pod holds no chat credential and speaks no chat
  protocol. This also shrinks the untrusted-input surface: the pod that runs the LLM no longer
  terminates a public-platform socket.

**Socket Mode, therefore no public ingress.** The router **dials out** to Slack over a WebSocket; no
inbound listener, no public URL, no TLS certificate, no request-signature verification endpoint. A
hub cluster on a private network with default-deny egress needs exactly one allowlisted destination
(`slack.com`), and nothing needs to be reachable from the internet. This suits kube-agents' private
clusters better than either the Slack Events API (a public HTTPS endpoint) or Google Chat's Pub/Sub
push, and it is why Socket Mode is the default rather than a variant (§7).

**Ingress and dispatch are independent — and this is what keeps Chat supported.** The router does
two separable things, joined by one normalized message:

| Stage         | What it does                                                                                                         | Platform-specific?                                     |
| ------------- | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| **Ingress**   | Terminate the platform connection; decode the platform event; extract text, principal, channel, thread               | **Yes** — one adapter per platform (Slack, Chat)       |
| **Normalize** | Emit the **`InboundMessage`** — the single internal shape, carrying the platform-qualified principal and `threadKey` | The seam. Below it, nothing knows which platform spoke |
| **Resolve**   | `/kage` verb → `@kage <handle>` → thread affinity → channel binding → NL inference, in that order                    | No                                                     |
| **Authorize** | Read the **target** agent's `allowedUsers` and check the normalized principal, **before** dispatch                   | No                                                     |
| **Dispatch**  | Deliver the `InboundMessage` on the existing per-agent transport to `<agent>.<ns>.svc`                               | No                                                     |

`InboundMessage`'s fields, and the field-by-field **Slack ↔ Chat parity table** that shows what each
adapter maps onto them, are specified in [06](06-api-and-data-contracts.md) §2b. Consult it when
adding or changing an adapter; nothing about it is restated here.

Adding Slack therefore added an **adapter**, not a second dispatch path — which is exactly why Chat
did not have to be removed, and why a spoke-cluster Developer Team Agent is reachable identically
whether the human is on Slack or Chat. Dispatch does not know, and must never learn, which platform
a message arrived on.

**Addressing.** Slack registers slash commands **statically, per app**, so a per-agent slash command
(`/devteam-charlie`) cannot work for a fleet that grows a namespace at a time — every new agent would
require an app manifest change. The grammar is therefore **one** command, `/kage <verb> <target> …`,
plus `@kage <handle> <intent>` in a channel, plus a **channel binding**: a Slack channel bound to one
agent, so a bare message in it routes deterministically with no handle and no inference. Precedence
is `/kage` → `@handle` → thread affinity → channel binding → NL inference; **only the last spends an
inference call**, and on low confidence it clarifies rather than guessing. The `<tier>-<scope-leaf>`
handle grammar is unchanged and platform-neutral —
[06](06-api-and-data-contracts.md) §2b owns the exact wire grammar, command table, and principal
format; do not restate them here.

**Thread affinity** keys on Slack's `thread_ts`, normalized into the platform-qualified `threadKey`
on the `InboundMessage` and held by the session store
([06](06-api-and-data-contracts.md) §2b, §6), so a follow-up in the same thread reaches the same
agent without re-addressing and without inference. Chat's thread identifier normalizes into the same
field — one of the parity rows in 06.

**Principals are platform-qualified and immutable** — `slack:U0123ABCD`, `googlechat:users/123` —
never a display name and never an email, both of which a user can change and an attacker can spoof.
`allowedUsers` and the approval roster are written in the same form
([03](03-security-model.md) §4a).

**Approvals.** A gate prompt (F8 step 3) renders as **Block Kit buttons over the same Socket Mode
connection**, with a typed `/kage approve <action-id>` fallback that is always available. The button
is a **convenience, never an authorization**: the click delivers an `action-id` and a claimed
principal, and the **broker re-verifies the clicking user against the approval roster** before
releasing anything (F8 step 4). A forged or replayed interaction payload can therefore do no more
than an unauthorized typed command — which is nothing.

**The brake never depends on Slack.** `pause`, `freeze`, and `undo` are Kubernetes objects reachable
by `kubectl` and by the API (§1.5, [06](06-api-and-data-contracts.md) §2b.1). The `/kage` forms are
conveniences over them and must be verified to work with Slack unreachable (§8 CH9).

**Failure behaviour: the human entrypoint is down; the fleet is not.** Slack unreachable, the app
token revoked, or the router crash-looping produces exactly one effect — **humans cannot start new
conversations and cannot receive reports through chat**. Agents keep observing, deciding, acting,
verifying, journaling, and undoing (F6 is the flow with no human in it). Gate prompts queue and are
re-delivered on reconnect; a `gated` action that reaches its TTL while chat is down expires safely
rather than executing (F8 step 6). Reconnect is automatic with backoff (§6), and the outage is
visible as `kubeagents/chat_ingress_connected == 0` rather than as silence.

**The single-connection failure domain, stated honestly.** One connection means the router is a
**single point of failure for the human entrypoint** — not for actuation, not for the brake, and not
for the proactive loop, but real. **Scaling out is not the fix and must not be attempted:** a second
active replica either has its connection refused or splits ingress, and `maxConnections: 1` is a code
constant precisely so no operator can turn a capacity dial into a correctness bug. The mitigations
are therefore **lease-based leader election with a warm standby and fast failover**, a liveness probe
on **connection state** rather than on process health (a router holding the lease with a dead socket
is _not_ healthy and must lose the lease), and the `kubectl` and API paths that make every
operational command reachable without chat at all.

**Why this makes the router run at all.** Today `kage-router` is **parked at zero replicas**: the
provisioning script scales it down and annotates why, because it only ingests Google Chat and the
live install is **Slack-only** — so the component has never carried production traffic, and shipping
it at one replica would only crash-loop on missing Chat configuration. Slack-first ingress is what
takes the router off the shelf: with the fleet's Slack app terminating here, the router is on the
path of every human turn on day one of an install, which is also what finally exercises `C15`'s
allowlist check, audit surface, and routing modes in production rather than only in unit tests
(§7, [07](07-implementation-roadmap.md)).

## 2. Topology (hub-and-spoke)

```
        ┌──────────────────────── HUB CLUSTER (kubeagents-system) ─────────────────────────┐
        │  C1 controller + C-AS admission   C15 router (Slack)   C5 inference   C6 Minty   │
        │                                                                                  │
...
| `kube-agents-chatops-gateway`           | C15              | `agents` and `chatopsconfigs` `get/list/watch` (the routing index and the fleet chat config), its own leader-election `Lease`, and `actionrecords/status` approvals — it records a human's approve/reject decision and can do nothing else ([06](06-api-and-data-contracts.md) §4.3). It also mounts the fleet's **Slack app + bot tokens** referenced by `ChatOpsConfig` (§1.8), which are chat-platform credentials and carry no cluster or cloud authority whatsoever |
...
| ChatOps router (C15)                           | ✅ (shared; **1 active replica + lease-elected standby** — the fleet's one Slack Socket Mode connection, §1.8) |            consumed remotely             | `kubeagents-system` (+ the cluster-scoped `ChatOpsConfig` singleton) |
...
**F5 — Chat ingress & routing (human → agent).** The reference path is Slack; Google Chat is the
parity case (step 1 is the only step that differs). See §1.8 for the component detail.

1. **Ingress.** A human types in Slack. The event arrives on the router's **single outbound Socket
   Mode connection** — the router dialled Slack, so there is no public endpoint and nothing inbound
   to the hub. _(Chat parity: the same event arrives on the Pub/Sub pull subscription. Everything
   from step 2 on is byte-identical.)_
2. **Normalize.** C15 decodes the platform event into **one internal message**: the text, the
   **platform-qualified principal** (`slack:U0123ABCD`; `googlechat:users/123` on Chat — never a
   display name or email), the channel, and a platform-qualified **`threadKey`** derived from Slack's
   `thread_ts`. Below this seam nothing knows which platform spoke.
3. **Resolve the target agent**, in strict precedence and stopping at the first hit: a **`/kage`
   verb naming a target** → an explicit `@kage <handle>` → **thread affinity** on `threadKey` →
   **channel binding** (this channel is bound to exactly one agent, so a bare message routes with no
   handle) → **NL inference**. Only the last spends an inference call; on low confidence it clarifies rather
   than guessing. The handle grammar is `<tier>-<scope-leaf>`, derived from the same `(tier, scope)`
   key the cardinality webhook enforces, so there is no routing table to drift
   ([02](02-agent-personas.md) §2.4, [06](06-api-and-data-contracts.md) §2b).
4. **Authorize, before dispatch.** C15 reads the **target** agent's `allowedUsers` and checks the
   normalized principal against it. Fail-closed: an empty or absent allowlist refuses everyone. This
   happens whether the target was resolved deterministically or inferred.
5. **Dispatch, platform-neutrally.** The internal message goes to the addressed agent over the
   **existing per-agent transport** to `<agent>.<ns>.svc` — the same delivery contract C16/C17 speak
   into the in-pod session-inject seam. **Dispatch is not aware of the ingress platform**, which is
   what lets a spoke-cluster Developer Team Agent be reached identically from Slack or from Chat, and
   what let Slack be added as an adapter rather than as a second dispatch path.
6. **Return.** C15 is also the **return path** for gate prompts (F8 — Block Kit buttons on the same
   connection, with a typed fallback; the broker still re-verifies the clicking user against the
   roster, §1.8), action reports (F1 step 11), and `undo` invocations (F7), replying in the
   originating thread.

Routing is a convenience, **never** an authz signal: a mis-route lands only on an agent the human may
already reach, still bounded by that agent's scope ceiling and gates. Every turn is audited with the
requester, the resolved agent, and the routing mode ([06](06-api-and-data-contracts.md) §2b). This is
human→agent dispatch, **not** agent-to-agent traffic, which uses the mesh (F3). And F5 is **not** on
the brake's critical path: with Slack unreachable this whole flow is unavailable while `pause`,
`freeze`, and `undo` remain reachable by `kubectl` and the API (§1.5, §8 CH9).
...
- **ChatOps router (C15):** see §1.8. Operationally: one hub Deployment running **one active replica
  plus a lease-elected warm standby** — **never** scaled beyond one active, because a second active
  replica splits ingress — holding the fleet's **single Slack app** and its **one** Socket Mode
  connection. Its configuration is the cluster-scoped **`ChatOpsConfig`** singleton `default`
  (token refs, Socket Mode settings with `maxConnections` pinned at 1 in code, `commandName: kage`,
  the Block Kit toggle; [06](06-api-and-data-contracts.md) §2b); the referenced app and bot tokens
  are Secrets in `kubeagents-system`, mounted nowhere else. Channel bindings and `allowedUsers` stay
  on the per-agent CRs. Slack is the default and needs exactly one allowlisted egress destination and
  **no inbound path at all**;
  Google Chat is enabled per install and adds a Pub/Sub pull subscription **without changing
  dispatch**. Readiness probes **connection state**, not process liveness — a router whose socket is
  dead is not healthy. `kubeagents/chat_ingress_connected`, reconnect counts, and routing-mode
  counters belong on the same dashboard as the SLIs (§5.1), because a silent chat outage and a quiet
  week are otherwise indistinguishable.
...
- **Cross-cluster connectivity (spoke → hub):** spokes consume the hub's inference (C5), Minty (C6),
  ...
  broker, and MCP grounding endpoints. **C15 is the one of these the hub _initiates_:** chat dispatch
  is hub → spoke onto the agent's own `Service`, so the corresponding rule is an ingress allowance on
  the spoke agent, not a spoke egress. It is the same rule regardless of whether the human was on
  Slack or Chat (§1.8, F5 step 5). Authn is the LiteLLM virtual key for inference, Workload
...
| **Chat turn latency**      | p95 < 10 s for read/plan; **p95 < 20 s from message to "done + undo handle"** for a routine single-object mutation. All four deterministic modes — `/kage`, `@handle`, thread affinity, channel binding — add **no** inference; only the NL fallback adds one router call                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Mutations are now synchronous, so they are inside the user-visible budget. Channel binding exists partly so the common case never pays for inference                                                                                                                                                             |
| **Chat ingress**           | Socket Mode connection established **< 10 s** from pod start; **reconnect after a dropped socket p95 < 5 s, p99 < 30 s** with jittered backoff; platform event → `InboundMessage` **p95 < 250 ms**; **no event lost across a reconnect**. Readiness probes **connection state**, not process liveness. **One active replica, always** — leader-election lease duration 15 s / renew 10 s, so **failover from a dead leader to the standby holding the connection completes p95 < 20 s**, and **zero windows with two live Socket Mode connections** is an invariant rather than a target. Ingress availability **≥ 99.5%** on `kubeagents/chat_ingress_connected`, measured and alerted **separately from broker availability**                                                                                                                                                                                                               | One app token permits **one** connection (§1.8), so it is a single failure domain for the human entrypoint — it must heal fast and be loudly down when it does not. Keeping it out of the broker's availability number is deliberate: a chat outage is not an actuation outage, and merging them would hide both |
...
- **Slack is the reference chat platform; Google Chat stays fully supported (v1).** Every default,
  example, and provisioning path is Slack-first because that is what real installs run — the live
  deployment is Slack-only — and a reference platform that nobody runs is a specification nobody
  tests. Chat is **opt-in, not deprecated**: it is a second ingress adapter above an unchanged
  normalizer and an unchanged dispatch path (§1.8, F5), so supporting it costs one adapter rather
  than a parallel system. **Consequence worth stating: this is what makes the router run in
  production.** `kage-router` is parked at **zero replicas** today precisely because it ingests only
  Chat while the live install is Slack-only, so the component has never carried production traffic.
  Moving Slack ingress into it puts it on the path of every human turn from day one of an install.
- **Socket Mode, not the Slack Events API (v1).** Three reasons, in order. (1) **No public ingress.**
  Socket Mode is an outbound WebSocket the router dials; the Events API requires a publicly
  reachable, TLS-terminated, signature-verifying HTTPS endpoint. kube-agents' hub is a private
  cluster with default-deny egress, and "one allowlisted outbound destination" is a far smaller ask
  than "an internet-facing endpoint in the control-plane namespace" — the same reasoning that keeps
  the mesh and the brokers internal-only. (2) **It works behind private networking** with no load
  balancer, no DNS record, no certificate lifecycle, and no request-signature verification code to
  get wrong. (3) **The single-connection limit is centralised rather than fought.** An app token
  permits exactly one Socket Mode connection; that is the constraint that made the previous
  generation's per-pod Slack relay unable to serve more than one tier (hence `slack.enabled: false`
  on every child). Holding the one connection in `C15` converts the limit into the router's reason
  for existing, and the cost — one active replica, a single failure domain for the human entrypoint —
  is bounded by leader-elected failover and by the `kubectl`/API paths that keep every operational
  command reachable with chat down entirely (§1.8, §6). The Events API remains the fallback if a
  deployment ever needs multiple concurrent ingress processes; it is not the v1 shape.
- **The router runs one active replica behind a lease, and `maxConnections` is a constant (v1).**
  The obvious operational instinct — scale the ingress Deployment for availability — is **wrong here
  and fails quietly**: a second active replica opens a second Socket Mode connection and splits
  ingress, so turns land unpredictably and neither pod reports unhealthy. HA is therefore a
  **lease-elected warm standby**, not horizontal scale; `maxConnections: 1` is pinned in code rather
  than exposed on `ChatOpsConfig`, so no operator can convert a capacity dial into a correctness
  bug; and liveness is measured on **connection state**, so a leader with a dead socket loses the
  lease instead of holding it (§1.8, §6, §8).
- **Fleet chat configuration is a cluster-scoped `ChatOpsConfig` singleton, not per-agent (v1).**
  There is exactly one Slack app for the fleet, so exactly one object describes it: `ChatOpsConfig`
  named `default`, holding the token references, Socket Mode settings, `commandName`, and the Block
  Kit toggle. Per-agent CRs keep only what is genuinely per-agent — channel bindings and
  `allowedUsers`. Putting app-level credentials on an `Agent` CR is what the retired per-pod relay
  did, and it is what made the one-connection limit look like a per-agent configuration problem
  instead of a fleet-level architectural one ([06](06-api-and-data-contracts.md) §2b).
...
- **Chat ingress is centralised, and the per-pod relay is gone:** exactly one workload in the fleet
  mounts a Slack app or bot token — `kage-router` in the hub (§1.8). Assert that **no agent pod** has
  a chat credential in its env, its projected volumes, or its rendered config, and that no `Agent`
  CR still carries a per-agent Slack relay block — fleet chat config lives only on the cluster-scoped
  `ChatOpsConfig` singleton `default`. Then assert the router is **running and serving**, not parked
  at zero, with **exactly one replica holding the leader lease and one Socket Mode connection open
  fleet-wide**; scale the Deployment to 2 and assert the second pod starts, takes no lease, and opens
  **no** connection. Finally assert readiness reflects **connection state**: kill the socket without
  killing the process and the pod must go NotReady and drop the lease, and the standby must take over
  inside the §6 failover budget.
...
**CH9 — No cascade, and the brake survives everything.** With inference, **Slack and the C15 router**,
and the hub all unavailable simultaneously — sever the Socket Mode connection outright, not just the
pod — assert first that the fleet keeps working: agents continue to observe, act, verify, journal,
and undo (F6 has no human in it), and only the **human entrypoint** is down. Then exercise the brake
with chat gone: `kubectl patch` a `pause` and assert the target broker refuses new
