...
| ChatOps router (C15)                           | ✅ (shared; **1 active replica** — the fleet's one Slack Socket Mode connection, §1.8) |            consumed remotely             | `kubeagents-system`              |
...
- **ChatOps router (C15):** see §1.8. Operationally: one hub Deployment, leader-elected, **one
  active replica**, holding the fleet's **single Slack app** — app token and bot token as Secrets in
  `kubeagents-system`, mounted nowhere else — and its **one** Socket Mode connection. Slack is the
  default and needs exactly one allowlisted egress destination and **no inbound path at all**;
...
- **Chat ingress is centralised, and the per-pod relay is gone:** exactly one workload in the fleet
  ...
  CR still carries a per-agent Slack relay block. Then assert the router is **running with one active
  replica**, not parked at zero, and that its readiness reflects **connection state**: kill the
  socket without killing the process and the pod must go NotReady.
