...
- `config/integrations/litellm/` — LiteLLM Deployment + Service (plus `NetworkPolicy` and `PodMonitoring`), with three overlays: `chatgpt` and `vertex`, which `MODEL_PROVIDER` selects in place of the base, and `custom-otel`, a copy-and-edit overlay for a collector other than the managed one ([Telemetry](/kube-agents/deploy/telemetry/) is canonical for it). These are the one subtree here whose `kustomize build` output is a template rather than a manifest: it carries `envsubst` placeholders and has to be substituted before `kubectl apply` will take it. `make deploy-litellm` does that; building an overlay by hand needs the pipeline on the Telemetry page.
