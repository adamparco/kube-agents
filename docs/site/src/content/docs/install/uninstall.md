...
The script runs the `teardown_13_*.sh` through `teardown_01_*.sh` steps in order, undoing each provisioning step. It reads state from `vars.sh` (created during provisioning) so you don't need to re-answer prompts.
...
| Script                                   | Removes                                                           |
| ---------------------------------------- | ----------------------------------------------------------------- |
| `teardown_13_apply_network_policies.sh`  | Per-tier egress allowlist and the tenant default-deny floor       |
| `teardown_12_deploy_agent_tiers.sh`      | Cluster-admin and developer-team `Agent` CRs and their identities |
...
| `teardown_08_deploy_platform_agent.sh`   | Platform-tier `Agent` CR and rendered manifests                   |
