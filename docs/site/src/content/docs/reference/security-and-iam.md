...
### The self-improvement loop is a separate identity, and a separate GitHub account

Off by default, and rendered only when `selfImprovement.enabled` is true. The hourly investigator runs under its own KSA, `kubeagents-selfimprove`, bound to a GSA of the same name that holds three read roles on the project and nothing else — `roles/logging.viewer`, `roles/cloudtrace.viewer`, `roles/monitoring.viewer`. Not `roles/cloudtrace.user`, which reads as the natural choice and carries `cloudtrace.tasks.create` and the trace-scope writes. It holds no GKE role at all, so `gcloud container clusters get-credentials` fails for every cluster in the project including its own: there is no path from the loop to a cluster under management. The three read roles are project-wide, though, because none of those APIs scopes below a project — so the loop could otherwise read a managed cluster's telemetry without touching the cluster. The evidence CLI is what closes that: log queries carry a namespace clause, metric queries carry a cluster clause, and each validates the caller's own filter so it cannot escape the parentheses wrapping it.

Its Kubernetes grant is the upstream `view` ClusterRole bound with a **RoleBinding**, so it stops at the release namespace, plus read on `platformagents`/`agentplugins` and `get`/`patch` on one ConfigMap by name — the ledger it keeps its occurrence counts in. `view` excludes Secrets, and `pods/exec`, `pods/attach` and `pods/portforward` are excluded with it, so a read grant cannot become a shell in the agent's pod. There is deliberately no `create` on ConfigMaps: the API server ignores `resourceNames` on create, so any create grant would be a namespace-wide one.

Its GitHub identity has no GCP resource behind it at all. Under `mode: fork` or `mode: upstream` the loop authenticates as a robot account holding a classic personal access token, which you create out of band and put in a Kubernetes Secret named by `selfImprovement.github.patSecret`; the chart mounts that Secret into the CronJob's credential-proxy sidecar and never sees the value. It reaches no minter, and the network is what stops it: the loop's own NetworkPolicy renders no egress rule to the minter's Service, and the platform minter's ingress admits only `kubeagents.x-k8s.io/has-credential-proxy: "true"` while this pod carries `kubeagents.x-k8s.io/selfimprove: "true"`. So the agent's GitOps credential and the loop's are unreachable from each other in both directions, and either can be revoked without the other noticing. Under the default `report-only` mode no credential proxy is rendered and the loop has no GitHub identity whatsoever.

Weigh two things before switching a mode on. A classic token carries `repo` — read and write on every repository its account can reach — where a GitHub App installation could have been scoped per repository, so use a robot account that is a member of nothing else and treat its reach as the security boundary. And nothing in the install rotates or expires the token: it is a standing write credential in a Secret until somebody revokes it. Inside the pod, the credential-proxy deny policy is what keeps it to opening pull requests — no `merge`, no `review`, no `gh api` — and that is a check on argv rather than a permission boundary. [`docs/designs/self-improvement.md`](https://github.com/gke-labs/kube-agents/blob/main/docs/designs/self-improvement.md) §5.4 and §6 are canonical for the credential's design — why a token rather than a GitHub App, what the sidecar mount buys, and every argv shape the deny policy refuses. What the loop is permitted to do stays this section's, as the note under its heading says.
