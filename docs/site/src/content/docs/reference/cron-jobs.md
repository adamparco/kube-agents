...
| `deliver` (optional) | string          | Chat delivery mode. `"local"` on all seven jobs: the Platform Agent profile carries no `platforms:` section and no chat transport, and reports reach humans by a route that depends on the job — the six audits through the [fleet-audit ledger issue](/kube-agents/concepts/autonomous-watchdogs/), `github-issue-resolver` through the individual issues it triages, and a dispatched run through the response the Chat Agent relays. `"all"` would fan out to whichever platform happened to hold a home channel at fire time, which is a routing decision nobody gets to review. |
