...
1. The Chat Agent profile's gateway — the only ticking one, since cron ticking belongs to a running gateway — evaluates its own `jobs.json` on schedule. The governance watchdogs live there: each is a `no_agent` script that files one kanban card assigned to `platform`, carrying that job's pre-authored `prompt` verbatim (see [How a watchdog fires](/kube-agents/concepts/autonomous-watchdogs/#how-a-watchdog-fires)). The same store's `profile-cron-tick` job also ticks every named profile's cron store once a minute, so anything scheduled on the Platform Agent's own `agents/platform/cron/jobs.json` fires from the same gateway, its prompt delivered as a synthetic message rather than a card. Nothing shipped takes that route — every relocated watchdog stays on that roster as a disabled tombstone, so it cannot fire both there and on a card; see [What fires the schedule](/kube-agents/concepts/autonomous-watchdogs/#what-fires-the-schedule).
2. A Platform Agent worker takes the card (or, for a job an operator schedules on the platform roster itself, the Platform Agent receives the prompt directly). The prompt is usually "read `governance/<sop>.md` in your profile home and execute"; the path is relative to the profile home, where `profile_scaffold.py` overlays the baked `/opt/platform-template/governance/` directory.
