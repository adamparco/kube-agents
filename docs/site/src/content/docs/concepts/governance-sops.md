...
GCP networking fabric & VPC IPAM, daily. Five checks over VPC networks, subnets, Cloud NAT gateways, Private Service Connect endpoints, and Cloud Armor security policies: subnet primary/secondary IP range exhaustion, Cloud NAT port allocation saturation, PSC endpoint routing deadlocks, VPC MTU packet fragmentation mismatches, and Cloud Armor policy evaluation anomalies.

Remediation for this stream may be a Terraform file as well as a Kubernetes manifest — the objects it audits (VPCs, subnets, NAT gateways) live in the cloud project, not the cluster. Invoked by the `gcp-networking-fabric-audit` watchdog.
