...
One more red line keeps the two write paths from blurring into each other. Before it pushes, `submit` diffs the branch against the base and asks GitHub which files the open audits already claim — the diff of every open `audit:remediation` pull request, plus the paths each open [ledger issue](#tier-1--the-ledger-issue) records in a hidden block — and refuses the overlap, naming what owns each file. Without it the agent could hand-roll a fix against a ledger it had just read, and nothing downstream would dedupe or ever close the result: one workload's findings produced five near-duplicate unlabelled pull requests that way before the check existed.

The signal is the diff rather than the description, which is the second design of this check and not the first. Matching closing keywords in the title and body caught two of those five — one referenced no issue at all, and two buried the reference mid-sentence — and it pointed the incentive the wrong way, since deleting the reference was then the cheapest way past the refusal. Files are not editorial, and they are what `fleet-audit` already keys its branches on, so the two agree by construction. Two cases are deliberately not refused: pushing to the audit's own remediation branch, which is how review feedback on one of its PRs gets addressed, and a repository whose GitHub is unreachable — this is routing, not a security boundary, so it fails open with a log line rather than halting every GitOps write in the Pod.
