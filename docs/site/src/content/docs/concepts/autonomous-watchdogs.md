...
- **A job will not run concurrently with itself.** A tick takes `cron/.job-<id>.lock` for the length of a run, so a schedule that fires while its last run is still going is skipped rather than doubled. See [Overlap and backlog](#what-fires-the-schedule).
...
Five watchdogs — `blueprint-sync`, `policy-propagation`, `global-capacity-orchestrator`, `standardization-validator`, and `lifecycle-deprecation-manager` — shipped disabled for several releases and no longer run. As written none could produce a finding on a stock install: two compared clusters against a "master blueprint" document no install provides, one read policy templates from an unshipped `/opt/defaults/templates/`, one ran hourly with no defined output artifact, and one overlapped `security-patch-orchestrator`.
...
They stay in the shipped roster as **tombstones** — full entries with `enabled: false` and no `skills` — because `merge_cron_store` adds and overwrites but never prunes. Deleting an entry therefore only ends the image's ability to hold the job off: a volume provisioned while the job was enabled keeps firing it. Shipping it disabled is what actually stops it, on every volume, at the next pod restart. An id is safe to delete outright only once every live volume has merged that disabled form. `cronjob(action='list')` against the Platform Agent lists the tombstones alongside the seven live watchdogs.

For the case that rule cannot cover — an id that has to stop firing on volumes that already have it _enabled_, in one release — `profile_scaffold.py` has `retire_cron_jobs`, exposed as the entrypoint's `--cron-retire`. It runs after the merge and deletes the named ids from the store outright, so the entry is gone at the next pod restart whatever state the volume was in. It is the exception, not the tool for an ordinary retirement: for as long as an id is on that list it cannot be scheduled on that profile at all, because the deletion outranks anything the image ships. The Chat Agent's roster uses it for the seven governance ids, which had to stop firing there the moment they resumed firing on the Platform Agent's.
