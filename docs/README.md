...
| `docs/designs/fleet-audit-collectors-and-status.md` | Feature design | Moves audit check execution from LLM inference into per-stream procedural collectors, adds native run timing, and keeps each run's report on the volume, where `make fleet-audit-view` and a reader skill project it. | Collector framework, run manifest, timing instrumentation, report store, run lock, view tool | **Draft**; not yet implemented (banner in file) |
