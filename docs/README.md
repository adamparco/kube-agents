...
| `docs/designs/fleet-audit-collectors-and-status.md` | Feature design | Moves audit check execution from LLM inference into per-stream procedural collectors, adds native run timing, and surfaces run health in a status ConfigMap read by `make fleet-audit-view`. | Collector framework, run manifest, timing instrumentation, status ConfigMap, view tool | **Draft**; not yet implemented (banner in file) |
