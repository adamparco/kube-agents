...
| `docs/designs/fleet-audit-collectors-and-status.md` | Feature design | Moves each stream's check execution out of LLM inference into a procedural collector, times a run natively off `finish`'s exit JSON, and keeps every run's report on the volume, where `make fleet-audit-view` and the `fleet-audit-reports` skill read it back in place of the retired ledger-body read-back. | Collector framework, run manifest, timing instrumentation, report store, run lock, `fleet-audit-view` | Amends `fleet-audit-issue-ledger.md`; implemented apart from the deferred `AuditSpec` consolidation (banner in file) |
