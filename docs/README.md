...
| `agents/selfimprove/SOUL.md` | Runtime persona | Persona of the self-improvement loop's investigator: an inward-looking reader of kube-agents' own source, harness and running install, with the read-only red lines that separate it from the Platform Agent (never mutate the cluster, never touch the fleet under management, findings go to a ledger and nowhere else under `report-only`). | Inward scope, read-only red lines, evidence-before-claim | Scaffolded onto an emptyDir by `selfimprove_run.py` at the start of each run; never a live profile |
| `agents/selfimprove/skills/self-investigation/SKILL.md` | Skill bundle | How one investigation turn works: the seven signal classes, which evidence source answers each, how to grade a finding by severity, and the `findings.json` contract the runner reads back. | Signal classes, evidence sources, severity rubric | Runtime doc for the loop's investigate turn |
| `agents/selfimprove/skills/file-pull-request/SKILL.md` | Skill bundle | How one promoted finding becomes an upstream pull request: branch, the five required body sections (finding, evidence, fix and rationale, live validation, code changes), and the mode difference between `fork` and `upstream`. | PR body contract, branch naming, fork vs upstream | Runtime doc for the loop's filing turn; unreachable under `report-only` |
| `terraform/modules/kube-agents-selfimprove/README.md` | Component README | Reusable Terraform module for the self-improvement loop's Google identity: the investigator GSA with three read-only telemetry roles (and deliberately no `container.*` role), plus a separate minter GSA and import-only KMS key behind `create_minter` for the modes that open pull requests. | Investigator GSA, two-account split, import-only KMS key | Infrastructure engineers; not yet wired into `examples/full-install` |
