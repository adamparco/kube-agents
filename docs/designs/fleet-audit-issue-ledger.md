...
| Ledger issue body    | Scope, findings table with state column and a link from each id to its detail, then per-finding detail: evidence, impact, its own id, recommendation, remediation, PR link. Hidden `<!-- audit-findings -->` marker listing the ids the body rendered, then the `<!-- audit-id-scheme -->` stamp that says which identity scheme minted them, and last `<!-- audit-paths -->` — every file the run's live findings would rewrite, not only the rendered subset, which `submit-suggestion` reads to refuse a hand-rolled duplicate. |
