...
# Every scheduled cron job lives in the Chat Agent's roster, because the
# `default` profile owns the only running gateway and cron ticking is a
# property of a gateway. The Platform Agent's roster holds the same governance
# jobs as disabled tombstones; the live copies are here, dispatched to it as
# kanban cards. This page therefore generates from the Chat Agent's file only.
