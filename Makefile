...
# without editing this file. Four globs rather than one because the tests do
# not all live under skills -- the agent scripts the skills share, the Chat
# Agent plugins and the image patches each hold their own. Discovery is then
# run once per directory rather than once over the tree, because none of them
# are packages -- `unittest discover` pointed at agents/platform/skills finds
...
test-python:
	@if [ -z "$(PYTHON_TEST_DIRS)" ]; then \
		echo "Error: no test_*.py files found under agents/ or deploy/docker/patches."; \
