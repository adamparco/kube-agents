...
| `python3 -m unittest discover local-dev`                                                 | L0/L1 | GitOps skill tooling: `submit-suggestion`, `read-knowledge`, `raise-escalation`, `detect_drift`, heartbeat SOPs                                                                        |
| `python3 local-dev/okf-validate.py examples/gitops-repo/knowledge`                       | L0    | OKF knowledge entries typed, links resolve (negative control included)                                                                                                                 |
...
| `local-dev/tests/iac-parity.py`                                                          | L0    | KCC ↔ Terraform HCL structural validity + semantic equivalence; `apply.yml` per-format dispatch                                                                                        |
| `local-dev/tests/circleci-parity.py`                                                     | L0    | Second CI/CD dispatch parity; no agent-held write credential                                                                                                                           |
| `local-dev/tests/observability-seam.py`, `local-dev/tests/otel-endpoint.sh`              | L0    | Provider-neutral observability seam; env override, unset default, fail-loud                                                                                                            |
| `local-dev/tests/negative-attenuation.sh [ctx]`                                          | L2    | **03 §11 load-bearing**: write-verb Role DENIED, `impersonate` ClusterRole DENIED, wrong-scope DENIED, read-only ADMITTED; distinguishes a policy denial from a malformed-object error |
| `local-dev/tests/egress-enforcement.sh [ctx=kind-kube-agents-egress]`                    | L2†   | Real egress enforcement on Calico. **Defers loudly** if the CNI does not enforce NetworkPolicy                                                                                         |
| `local-dev/kind/chaos-suite.sh [ctx]`                                                    | L2    | **05 §8 load-bearing**: C1 controller-down, C2 relaunch, C3 no-cascade, C4 hub-down                                                                                                    |
| `local-dev/kind/verify-phase<N>.sh [ctx]`                                                | L2    | Phase `N` consolidated gate + regression of `N-1`. Current chain: 7 → 6 (+chaos) → {5,4,3,2} → 03 §11 → goldens → `go test`                                                            |
| `local-dev/kind/up.sh` / `down.sh`                                                       | —     | Bring the inner-loop Kind cluster up/down                                                                                                                                              |
| `local-dev/gke-scratch/create.sh` / `destroy.sh`                                         | —     | Create/destroy the ephemeral scratch GKE cluster                                                                                                                                       |
...
| **L1** | Process-local                                                             | `go test ./...`, envtest, `python3 -m unittest discover local-dev`                                                                                                    |
| **L2** | **Kind `kube-agents-dev`**                                                | context **`kind-kube-agents-dev`**, `kindest/node:v1.31.2`, config `local-dev/kind/kind-config.yaml`. **kindnet does not enforce NetworkPolicy**                      |
| **L2** | **Kind `kube-agents-egress`** (NetworkPolicy)                             | context **`kind-kube-agents-egress`**, `local-dev/kind/kind-calico.yaml` + Calico v3.28.0. The **only** Kind target where an egress claim may be green                |
...
| P3  | **No grandfathered objects.** Force-recreate the pods/objects the property is about; admission policies and PSA do not evict what already exists.                                                                      | LSN-002 |
