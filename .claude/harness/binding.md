...
| `local-dev/tests/egress-enforcement.sh [ctx=kind-kube-agents-dev]`                       | L2†   | Real egress enforcement on Calico. **Defers loudly** if the CNI does not enforce NetworkPolicy                                                                                         |
...
| `local-dev/kind/up.sh` / `down.sh`                                                       | —     | Bring THE inner-loop Kind cluster up/down: 2 nodes, Calico, operator, VAP, agent images. Replaced `up-egress.sh` and `up-2node.sh`                                                     |
...
| **L2** | **Kind `kube-agents-dev`** — the only inner-loop cluster                  | context **`kind-kube-agents-dev`**, `kindest/node:v1.31.2`, config `local-dev/kind/kind-config.yaml`, built by `local-dev/kind/up.sh`. **2 nodes** (RWO excludes per node — LSN-015) and **Calico v3.28.0** (kindnet enforces no NetworkPolicy — LSN-006/P4). Was three clusters until 2026-07-26; they differed only in CNI and node count, which are orthogonal, so this is their union |
