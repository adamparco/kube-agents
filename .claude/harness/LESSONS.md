...
| **LSN-007** | completeness, wiring   | Every unit test passes and the feature does nothing in a real install                   | closed   | `install-path-wired.py` · V-CMP-004 · 09 §5.1 _(re-closed P8-T3)_ |
...
| **LSN-015** | fixtures, topology     | Everything works with one of a thing and deadlocks with two                             | **open** | **Bound** to Phase 8 Accept (a) / P8-T4                           |
...
**Open: 4 of 19.** Threshold is 5 (`binding.md` §Thresholds).
...
| **Mechanization** | `local-dev/tests/install-path-wired.py` — every numbered step script is invoked by its driver, every driver reference resolves, provision/teardown are symmetric, teardown descends. Plus **V-CMP-004** (`replicas > 0` in the default install) and **V-CMP-003** (no `REPLACE_WITH_*` in a shipped manifest); 09 §5.1, §11.9.                                                                                                                                                                                                                                                                                                                                                                                   |
...
| **Verify**        | Park a required Deployment at 0 replicas and confirm V-CMP-004 fails. For the wiring half: comment out a `provision_NN` line in `provision.sh` and confirm `install-path-wired.py` exits non-zero (this is one of its seven self-test controls).                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| **Mechanization** | **Bound, not yet proven** (2026-07-25). Phase 8 Accept (a) is bound to **V-CMP-001 / V-CMP-004 / V-CMP-005** in `docs/build/phase-8.md`, and the fixture rule is recorded there under **P8-T4** and in "Notes carried into IMPLEMENT": _any per-agent resource is exercised with **≥2 agents in one namespace**_. Closes when P8-T4's fixture actually runs two agents in one namespace and the `system-metadata` PVC no longer multi-attaches. |
