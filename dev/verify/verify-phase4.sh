...
if python3 dev/okf-validate.py "$TMP_BAD" >/dev/null 2>&1; then bad "okf-validate PASSED a frontmatter-less entry (validator is a no-op)"; else pass "okf-validate negative control: frontmatter-less entry REJECTED"; fi
rm -rf "$TMP_BAD"
...
if $K version >/dev/null 2>&1; then
  ...
  if bash dev/tests/negative-attenuation.sh "$CTX" >/tmp/p4-neg.log 2>&1; then
