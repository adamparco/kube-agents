...
if [ "$egr_up" -eq 1 ]; then
  run_l2 tenant-isolation dev/verify/tenant-isolation-l2.sh "$EGR_CTX" \
    "the ResourceQuota and namespace default-deny are applied BY PROVISIONING and bind"
  run_l2 gitops-tree dev/verify/gitops-tree-applies-l2.sh "$EGR_CTX" \
...
if [ "$dev_ok" -eq 1 ]; then
  run_l2 multi-agent dev/verify/multi-agent-namespace-l2.sh "$DEV_CTX" \
...
if [ "$dev_ok" -eq 1 ]; then
  run_l2 closed-allowlist dev/verify/closed-allowlist-l2.sh "$DEV_CTX" \
...
NEG="dev/verify/webhook-negatives-l2.sh"
if [ -f "$NEG" ]; then
...
if [ "$dev_ok" -eq 1 ]; then
  if bash dev/verify/verify-phase7.sh "$DEV_CTX" >/tmp/p8-regress.log 2>&1; then
