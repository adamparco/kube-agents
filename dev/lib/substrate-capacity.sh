...
#   survive a substrate change without an edit on both sides: when the last Kind creator goes, the
#   Kind preflight and its `@covers:` line go with it, the map shrinks, and no check has to be
#   remembered. A hardcoded table in the gate would instead go quietly green about a pattern that
#   no longer exists — the failure mode every VACUOUS arm in that file exists to prevent.
#
...
# --- knobs, local substrate (retired with the last `kind create cluster` caller) -------------------
