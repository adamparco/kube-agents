...
| `10-controller/`   | kube-agents `config/install` (CRD + controller + webhooks + `kage-router` + mesh CA), pinned to a release ref | The per-cluster control plane (05 §7). Reconciles the Agent pod; the router is the read-only ChatOps front door. Pipeline waits for the controller Deployment. |
