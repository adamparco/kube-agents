...
for that cluster and reports the files it wrote. It submits nothing, and there is no agent-side
submission path: `submit-suggestion` has been deleted from every tier, and the Action Envelope path
that replaces it lands in P11-T4 on top of the broker-side tier-template renderer from P10-T0 (see
SKILL.md, "Why this still renders a bundle", and the B-001 · B-002 ruling in docs/build/BACKLOG.md).
A human takes the bundle forward, reviews and merges; the customer's CI/CD applies it in bootstrap
order.
...
makes no cluster/cloud mutation. All actuation is a human's review + the CI/CD pipeline.
