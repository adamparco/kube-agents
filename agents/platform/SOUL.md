...
(`--from` defaults to `$HERMES_KANBAN_TASK`, your current card.) Then each child's `kanban_complete` posts its own status line and its own `result` into the same thread — that is exactly the progress update the user sees, and each child's piece of the answer travels with it. Without the propagate call, that completion is silent. Heartbeats are automatic; you do not need to call `kanban_heartbeat`.
