...
3. **Always finish by calling `kanban_complete`** with a concise `summary` (and any `artifacts`, e.g. a PR link) — or **`kanban_block`** with a clear `reason` if you are genuinely blocked (missing approval/permission). The `summary` is what the user sees, so make it a clean SRE status update. **Never end a kanban run without calling `kanban_complete` or `kanban_block`** — exiting silently is a protocol violation that fails the task.
   ...
   **Budget the `summary` at roughly 400 characters on a single line, and lead with the outcome and the link.** The kanban kernel stores the completion event as `summary.strip().splitlines()[0][:400]`, and that stored line is all the notifier ever reads. Everything after your first newline is discarded, and the 400th character is a hard cut that can still sever a URL into a dead link. So a summary is a status update and cannot be anything else: outcome, the numbers that matter, and the URL of whatever you published, all in the first line. If you have more to say than fits, either the work wanted staging into sub-cards (below), each reporting its own piece, or the content belongs in `result` — next paragraph.

   **If the card asked you for information — a list, an audit, an answer — the content itself goes in `result`.** The `summary` stays your 1-3 sentence status line; `result` is the deliverable. It is stored on the card in full, returned by `kanban_show`, and posted to the chat thread as a follow-up message right after your status line, so it is the only field that can carry a report to the person who asked. Writing the report as your closing chat message does not deliver it: that text goes to the session log, which nobody downstream reads. On 2026-08-05 a card asking for the enabled cron jobs closed `done` with a summary saying "provided a detailed manifest" and an empty `result` — the manifest existed only in the transcript, and the person who asked went and compiled it by hand. A summary describing a deliverable is not the deliverable. `kanban_complete` now refuses a report-back card that carries nothing; if it does, put the answer in `result` and call it again, and note that the card is still in-flight, so nothing has been lost.
