...
**Finding ids must be stable across runs.** Format: `<check-id>--<cluster-or-project>--<object>`, lowercased, every non-alphanumeric collapsed to `-`, truncated to 100 chars, then any leading or trailing `-` trimmed off. The result must match `^[a-z0-9]([a-z0-9._-]{0,98}[a-z0-9])?$`, with no `..` segment and no `.lock` suffix: the id is the join key of the ledger's hidden delta block and of the `audit-persists:<id>` marker — both line-anchored regexes — and an operator types it by hand in `/remediate <id>`, so a colon, a space, a `*`, a `..`, or a `.lock` ending is rejected outright. Never embed a timestamp, a counter, a sample value, or an age in an id — the same problem must keep the same id so the helper's delta works.
...
- `CLEAN` with `partial: true` → nothing was found, but nothing was closed either. **Not** silent: one line giving the clean result, the `coverage_gaps`, and the issue URL.
- `UPDATED` with `new: 0` **and** `resolved: 0` **and** `partial: false` → nothing moved since last week. Also **exactly** `[SILENT]`. Silence needs all three; a run that skipped a project has news even when the numbers did not change.
...
- Never write outside the GitOps declaration of the object a remediation targets, `remediations/fleet-wide-cost-analysis/`, and the helper's `findings_path`. Never hand-write or edit the ledger issue body or a remediation PR body, and never call `gh issue create` or `gh pr create` — this stream has exactly one ledger issue and `finish` owns it.
