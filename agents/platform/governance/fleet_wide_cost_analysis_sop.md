...
- Every entry in `manifest.clusters` carries one `outcome`. `"collected"` means every check that applies to that target already ran — copy its `commands` list verbatim into that target's `checks_run`. `"unreachable"` or `"gate-failed"` means the collector could not cover it; fall back to this section's manual reads for it alone.
- A `"collected"` cluster whose `commands` is missing `overrequest` means `kubectl top` was unavailable there — the metrics degradation below. Write the `limitations` note this section already directs and skip only 3.1; every other check on that cluster is still fully collected.
- An Autopilot cluster's `commands`/`candidates` never include `idle-nodepool`/`scaledown-blocked` — the collector does not run them there, matching this section's Autopilot rule below. Declare `idle-nodepool` in `checks_not_applicable` as directed; `scaledown-blocked` still applies conceptually but has nothing to evaluate without 3.7's flagged nodes, so it is simply absent rather than declared.
- Every entry in a `"collected"` target's `candidates` is a verified finding: `check`, `object`, `severity`, and `excerpt` are already computed, including 3.1's Autopilot severity bump and 3.8's PDB-deferral to the Workload Reliability audit. What is still yours to write is the `recommendation` (§5) and, for a `kind: manifest` remediation, the manifest file itself (§4).
- Pass `--manifest-file <path>` to `finish` (§6) so it cross-checks your `checks_run` against what the collector actually ran.

**A target the collector covered is not a target you query again.** The commands below exist for the manual fallback and for confirming a candidate's evidence.

**Metrics degradation:** if `kubectl top nodes` exits non-zero, the cluster still belongs in `scope.clusters` — you read it. Set `"limitations": "metrics-server unavailable: <stderr excerpt>; usage check 3.1 did not run."` on that cluster's `scope.clusters` entry and skip **only** 3.1 there. Every other check reads requests and object state, not usage, and still runs.
