...
- **Severity:** `major`.
- **Impact:** two unrelated conditions share this check and no one sentence is true of both, so the collector writes the arm's own Impact onto the finding. State it as given rather than re-deriving it. What it writes:
