...
- Every entry in `manifest.clusters` carries `outcome: "collected"`. Copy its `commands` list verbatim into that cluster's `checks_run` — the slugs that actually applied there, never a slug this manifest does not list.
