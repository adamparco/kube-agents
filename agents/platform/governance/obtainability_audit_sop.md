...
- **Impact:** "the first evicted under node pressure" is a claim about QoS class, and the flag-when above is an **or** — so most of what this check catches is not the class that sentence describes. The collector computes the pod's real class and writes the matching Impact itself, so the sentence arrives already matched; state it as given rather than re-deriving it. What it writes, by class:
  ...
  - **Burstable** — everything else, which is most of what this check catches. `kube-proxy` and `antrea-controller` ship on every GKE cluster with a cpu request and no memory request.
