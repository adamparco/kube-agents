...
Returns `{"issue": <int|null>, "repo":"org/repo", "findings_path":"/opt/data/scratch/findings_obtainability-audit.json", "pending_remediation_requests":[…]}`. Keep `findings_path` and the repo working tree from this call; you write both.
...
- Every `remediation.kind == "manifest"` finding writes exactly one file to `remediations/obtainability-audit/<finding-id>.yaml` in the repo working tree **before** `finish` runs. Naming the file after the finding id guarantees uniqueness and makes the remediation PR's diff self-describing. The path is repo-relative POSIX: no leading `/` or `:`, no `..`, and no glob metacharacters (`*`, `?`, `[`, `]`). The helper rejects all of those, and hard-fails on a path that is not on disk.
...
- Writing the file does not open a Pull Request. `finish` opens one automatically only for a `critical` finding whose remediation is a `manifest` and whose branch has no PR in any state, at most five per run; the ledger names the ones it withheld. Everything else waits for a repo writer to comment `/remediate <finding-id>` or `/remediate all` on the ledger issue. In this audit only 3.4 produces a `critical` manifest, so that PDB rewrite is the one fix that normally arrives ready to merge. A `kind: manual` finding is never promotable — it stays prose in the ledger. Findings whose manifest paths intersect share one PR; naming each file after its finding id keeps them disjoint.
...
One JSON line comes back, carrying `status`, `issue_url`, `new`, `resolved`, `prs_opened`, and `prs_closed`. Exit 2 means the validator rejected the document and nothing was published — fix the document, do not retry blind. Exit 1 is fatal. Exit 0 means it published.

- `status: "CLEAN"` — the ledger issue is closed as completed and every open remediation PR for this stream is closed with it. Your entire final response is exactly `[SILENT]`. Nothing else, no preamble.
