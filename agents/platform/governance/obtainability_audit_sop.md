...
- **Impact:** "Zero-downtime is impossible: every rollout, node drain, and node repair is a full outage for this service." — **unless the candidate's `selecting services:` line ends in `(metrics scrape only)`**, exactly as in 3.9 and for the same reason: there is no user traffic to drop, only a scrape Prometheus retries. Say instead: "Every rollout, node drain, and node repair leaves this workload's metrics unscraped until the replacement pod is up, so its own dashboards and alerts go blind across the gap." Keep the severity at `minor` and do not title the finding for user-facing downtime.
