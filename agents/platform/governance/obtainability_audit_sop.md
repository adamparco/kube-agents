...
- **Impact:** "the first evicted under node pressure" is a claim about QoS class, and the flag-when above is an **or** — so most of what this check catches is not the class that sentence describes. Read the excerpt and pick the arm it puts you in.

  - Every effective container silent on both `requests` and `limits` — the pod is BestEffort, and the sentence holds: "The scheduler and cluster autoscaler size this cluster as if this workload costs nothing; its pods are the first evicted under node pressure and its cost cannot be attributed."
  - A container declares a **limit** for the resource whose request is missing. Kubernetes copies the limit into the request at admission, so nothing is unreserved — the pod is reserved at its ceiling and is Guaranteed, the _last_ thing evicted. Still a finding, because §3.1 wants the request declared rather than inferred from a ceiling, but the cost is held headroom and an unreviewed reservation, not free scheduling.
  - Some other container declares a request, and the missing one has no limit either — the pod is Burstable. The named resource is genuinely unreserved, so bin-packing and attribution both suffer, but the kubelet evicts it **after** every BestEffort pod on the node and ranks it by how far usage exceeds a request of zero. `kube-proxy` and `antrea-controller` ship in this shape on every GKE cluster; telling their owner they go first sends them looking for a pressure event that reached them before it reached anything else.

  The collector writes the second and third itself, so the sentence arrives already matched to the arm; state it as given rather than re-deriving it.
