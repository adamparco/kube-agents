...
# The real merge the entrypoint runs, not a re-implementation of it: the bug
# UpgradePathTest guards lives in the interaction between that function's id
# filter and the argument step 2c passes it, so a stand-in would agree with
...
ENTRYPOINT = REPO / "deploy/shared/docker-entrypoint.sh"
...
class DedupTest(DispatchHarness):
    ...
    def test_the_cap_counts_only_this_jobs_blocked_cards(self):
        # Seven jobs share the `platform` assignee and this creator, so a
        # fleet-wide count would let one wedged job stop the other six.
        self.list_response = json.dumps(
            [
                {"id": f"t_o{i}",
...
class AlertingTest(DispatchHarness):
    ...
    def test_a_missing_kanban_cli_alerts(self):
        ...
        # permanent, so failing open would retire all seven jobs silently.
...
class WiringTest(unittest.TestCase):
    """The shipped roster, the wrappers, and the tombstoned platform roster."""
    ...
    def test_the_platform_roster_holds_only_tombstones(self):
        # One schedule, in one file. Upstream could empty this roster outright,
        # because nothing there ticks the platform profile's store. Here
        # `profile-cron-tick` does, so an entry left enabled would fire from
        # this store *as well as* through the card the Chat Agent files — the
        # same audit twice, writing the same ledger issue. The entries stay as
        # `enabled: false` tombstones rather than being deleted because
        # `merge_cron_store` adds and overwrites but never prunes, so shipping
        # the disabled form is the only way to switch off the enabled copies an
        # upgraded volume already has.
        platform_jobs = json.loads(PLATFORM_ROSTER.read_text(encoding="utf-8"))["jobs"]
        self.assertTrue(platform_jobs, "no tombstones left; this test guards nothing")
        for job in platform_jobs:
            self.assertFalse(job.get("enabled", True), job["id"])

    def test_every_platform_tombstone_has_a_dispatch_entry(self):
        # A tombstone is only safe because the job still runs somewhere else. An
        # id disabled here and absent from the Chat Agent's roster is an audit
        # that was switched off, not relocated, and nothing else would say so.
        platform_jobs = json.loads(PLATFORM_ROSTER.read_text(encoding="utf-8"))["jobs"]
        self.assertEqual(
            sorted(j["id"] for j in platform_jobs),
...
class UpgradePathTest(unittest.TestCase):
    """Step 2c's merge, run against the volume of a cluster that already exists.

    Shipping the entries is only half of the relocation. `cp -ru` can never
    refresh `cron/jobs.json` — the scheduler rewrites it every tick, so the
    volume's copy is permanently the newer one — which leaves the entrypoint's
    per-id merge as the only road onto a live PVC, and it travels that road
    only for the ids `--cron-jobs` names.

    Getting that list wrong is silent in both directions, which is why it is
    tested against the real function rather than described in a comment:

      - too narrow and the seven never arrive, while step 2.6 merges the
        platform roster UNFILTERED and its `enabled: false` tombstones DO —
        so every governance watchdog stops firing on every upgraded cluster
        and nothing anywhere reports it. A fleet that is no longer audited
        reads exactly like a fleet with nothing wrong.
      - too wide and the onboarding pair, which deletes itself once the
        first-run report is delivered, comes back to poll once a minute
        forever against a marker.

    The ids come out of the entrypoint and the definitions out of the shipped
    roster, so neither can drift from what the pod actually does.
    """

    # `bootstrap_delivery.py` calls `remove_job` on both. They are `no_agent`
    # script jobs exactly like the seven, so "every no_agent job with a script"
    # is not a safe way to compute the allowlist — only naming ids is.
    SELF_DELETING = ("bootstrap-inventory-scan", "bootstrap-inventory-delivery")

    def setUp(self):
        self.image = json.loads(CHAT_ROSTER.read_text(encoding="utf-8"))
        self.dispatch_ids = sorted(
            str(j["id"])
            for j in self.image["jobs"]
            if str(j.get("script", "")).startswith("dispatch_")
        )
        self.allowlist = self.step_2c_allowlist()

    def step_2c_allowlist(self):
        """The ids step 2c passes, parsed back out of the entrypoint itself."""
        text = ENTRYPOINT.read_text(encoding="utf-8")
        lists = [ids.split() for ids in re.findall(r'--cron-jobs\s+"([^"]*)"', text)]
        named = [ids for ids in lists if "profile-cron-tick" in ids]
        self.assertEqual(
            len(named), 1, "expected exactly one default-profile cron allowlist in the entrypoint"
        )
        return tuple(named[0])

    def upgraded_volume(self, onboarding=True, extra=()):
        """A live roster shaped like the release before the relocation.

        The ticker and the reconcile sweep are there with run history on them,
        the governance jobs are not — on that release they lived on the
        platform profile's roster, which is where the tombstones now switch
        them off.
        """
        jobs = [
            {
                "id": "profile-cron-tick",
                "script": "profile_cron_tick.py",
                "enabled": True,
                "last_run_at": "2026-08-06T12:00:00Z",
            },
            {
                "id": "cluster-agent-reconcile",
                "script": "cluster_agent_reconcile.py",
                "enabled": True,
                "last_run_at": "2026-08-06T11:11:00Z",
            },
        ]
        if onboarding:
            jobs += [{"id": job_id, "enabled": True} for job_id in self.SELF_DELETING]
        return {"jobs": jobs + list(extra), "scheduler_state": {"tick": 9}}

    def merged(self, live):
        store = ps.merge_cron_store(self.image, live, self.allowlist)
        return {str(job["id"]): job for job in store["jobs"]}

    def test_an_upgraded_volume_learns_about_every_dispatch_job(self):
        # The regression this class exists for. Before the allowlist named
        # them, this returned a roster with none of the seven on it.
        merged = self.merged(self.upgraded_volume())
        self.assertEqual(
            [],
            [job_id for job_id in self.dispatch_ids if job_id not in merged],
            "a shipped watchdog that never reaches a live volume has been switched off, "
            "not relocated — and its platform-side copy is a disabled tombstone by now",
        )
        for job_id in self.dispatch_ids:
            with self.subTest(job=job_id):
                entry = merged[job_id]
                self.assertTrue(entry.get("enabled"), "arrived disabled; it will never fire")
                self.assertTrue(entry.get("no_agent"), "arrived as a model turn, not a subprocess")
                self.assertTrue(entry.get("schedule"), "arrived with no schedule to fire on")
                self.assertEqual(
                    entry.get("script"),
                    next(j for j in self.image["jobs"] if j["id"] == job_id)["script"],
                )

    def test_the_self_deleted_onboarding_pair_is_not_resurrected(self):
        # The volume of a cluster that finished onboarding: the pair removed
        # itself and must stay removed, on the same merge that delivers the
        # seven. Widening the list to "every no_agent job with a script" would
        # pass the test above and fail this one.
        merged = self.merged(self.upgraded_volume(onboarding=False))
        for job_id in self.SELF_DELETING:
            self.assertNotIn(job_id, merged, f"{job_id} came back after deleting itself")
        self.assertTrue(set(self.dispatch_ids) <= set(merged))

    def test_the_allowlist_names_no_job_that_removes_itself(self):
        for job_id in self.SELF_DELETING:
            self.assertNotIn(
                job_id,
                self.allowlist,
                f"{job_id} deletes itself at runtime; forcing it back is what the list avoids",
            )

    def test_an_operators_own_job_is_left_exactly_as_it_was(self):
        # Nothing in the image knows this entry exists, and the merge must not
        # start pruning to make room for the ones that do.
        mine = {
        ...
        merged = self.merged(self.upgraded_volume(extra=[mine]))
        self.assertEqual(mine, merged["nightly-backup-check"])
        self.assertTrue(set(self.dispatch_ids) <= set(merged))

    def test_a_dispatch_entry_already_on_the_volume_keeps_its_run_history(self):
        # Forcing is per key, so the second upgrade is not the first one over
        # again: the image's definition wins, and `last_run_at` — which for a
        # recurring job is what the scheduler's catch-up window reads — is a
        # key the image ships nothing for, so the volume keeps it.
        job_id = self.dispatch_ids[0]
        stale = {
            "id": job_id,
            "prompt": "the prompt the previous release shipped",
            "enabled": False,
            "last_run_at": "2026-08-06T06:20:00Z",
        }
        merged = self.merged(self.upgraded_volume(extra=[stale]))[job_id]
        image_entry = next(j for j in self.image["jobs"] if j["id"] == job_id)
        self.assertEqual(image_entry["prompt"], merged["prompt"])
        self.assertTrue(
            merged["enabled"],
            "the image owns `enabled` — the same key the platform tombstones use to switch "
            "a stale copy off, working here in the other direction",
        )
        self.assertEqual("2026-08-06T06:20:00Z", merged["last_run_at"])
