...
`agents/platform/cron/jobs.json` sat in its roster without ever firing. All
seven governance jobs now live in this profile's `cron/jobs.json`, which is the
roster the one running ticker reads. Their old entries stay behind, shipped
`enabled: false`: this profile also runs `profile_cron_tick.py`, which does
tick the Platform Agent's store, so an entry left enabled over there would fire
alongside the card filed from here.
...
seven pins a `model`, and `agent.max_turns: 250` is set on the platform profile
...
the six audits, whose deliverable is the `fleet-audit` ledger issue rather
...
`SOUL.md AGENTS.md CAPABILITIES.md` and not `cron/`. What closes that gap is
the per-id merge in step 2c, which hands this roster to
`profile_scaffold.merge_cron_store` under a `--cron-jobs` allowlist — the ids
the image is allowed to force onto a live volume. All seven dispatch entries
are named there, alongside `profile-cron-tick`, so an upgraded cluster gets
them on its next pod restart and nobody edits a jobs.json by hand. That
allowlist stays an explicit list rather than becoming "everything shipped"
because two of the jobs here — the onboarding scan/delivery pair — delete
themselves once the report is out (`bootstrap_delivery.py` calls `remove_job`),
and an unfiltered merge would resurrect them on every restart to poll forever
against a marker. The seven qualify because nothing at runtime removes them:
each one only files a card.

The platform-side copies need no hand-pruning either:
`profile_scaffold.merge_cron_store` adds and overwrites but never prunes, and
they are shipped `enabled: false` precisely so that overwrite switches off the
enabled copies an upgraded volume still holds. That half of the upgrade always
worked — step 2.6 merges that roster unfiltered — which is why an allowlist
that stopped at `profile-cron-tick` would have disabled the seven on every
existing cluster and delivered nothing in their place.
"""
...
def survey_board(job_id: str, job_name: str) -> tuple[bool, list[str], bool, int]:
    ...
    try:
        ...
        # here would retire all seven governance jobs behind one stderr line.
        raise
