...
# `local-dev/L2-CHAIN.txt` can run against it; there is no second target to remember.
#
# WHY ONE CLUSTER
...
if [ "$SKIP_AGENT_IMAGES" = "1" ]; then
  ...
  echo "     local-dev/kind/reload-images.sh agents $CTX"
else
...
====================================================================
Every line of local-dev/L2-CHAIN.txt targets this one cluster:
    while read -r c; do case "\$c" in ''|\#*) continue ;; esac; eval "\$c"; done < local-dev/L2-CHAIN.txt

Phase gate:   local-dev/kind/verify-phase8.sh $CTX
