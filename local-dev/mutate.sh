...
#
#   local-dev/mutate.sh FILE [FILE...] -- COMMAND [ARGS...]
#
...
# through, so this composes: `local-dev/mutate.sh f -- sh -c 'mutate && ! check'`.
set -uo pipefail

usage() {
  echo "usage: local-dev/mutate.sh FILE [FILE...] -- COMMAND [ARGS...]" >&2
  exit 2
}
