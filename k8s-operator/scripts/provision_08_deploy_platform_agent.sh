...
execute_custom_resource() {
  ...
  # Render the closed allowlists (06 §1.2 V-7). render_allowlist_block emits
  # nothing when the integration is disabled and exits non-zero when it is
  # enabled with an empty or all-blank list — the CR never carries a list that
  # names nobody.
  ALLOWED_USERS_BLOCK="$(render_allowlist_block "${GOOGLE_CHAT_ENABLED}" "${ALLOWED_USERS:-}" "Google Chat (ALLOWED_USERS)")"
  SLACK_ALLOWED_USERS_BLOCK="$(render_allowlist_block "${SLACK_ENABLED}" "${SLACK_ALLOWED_USERS:-}" "Slack (SLACK_ALLOWED_USERS)")"

  # Ensure variables are explicitly exported so envsubst can access them
  export PROJECT_ID REGION CLUSTER_NAME MODEL_DEFAULT_NAME MODEL_PROVIDER GSA_NAME CHAT_SUB_NAME CHAT_TOPIC_NAME GOOGLE_CHAT_MODE ALLOWED_USERS_BLOCK AGENT_IMAGE NAMESPACE KSA_NAME GOOGLE_CHAT_ENABLED SLACK_ENABLED SLACK_BOT_TOKEN SLACK_APP_TOKEN SLACK_ALLOWED_USERS_BLOCK SLACK_HOME_CHANNEL SLACK_HOME_CHANNEL_NAME AGENT_TAG GITHUB_FULL_REPO MEMORY_ENABLED MEMORY_PROVIDER USER_PROFILE_ENABLED
