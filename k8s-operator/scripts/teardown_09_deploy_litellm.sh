...
if [ "${DRY_RUN:-0}" -eq 1 ]; then
  ...
  # substituted config.yaml. Render with the wrong values and the delete asks
  # for a ConfigMap that was never created while the real one stays behind.
  #
  ...
  # VERTEX_PROJECT_ID/VERTEX_LOCATION fall back to empty and
  # DEFAULT_VERTEX_LOCATION respectively. On a vertex_ai install torn down
  # without a vars.sh — or with one written before these variables existed — the
  # checksum will not match the deployed ConfigMap's and that ConfigMap survives
  # the teardown. Deleting it by name is the fix: `kubectl get configmap -n
  # "$NAMESPACE" -l app.kubernetes.io/name=litellm` lists every generation. The
  # Makefile's own fallbacks do not help here; they exist to keep the rendered
  # stream parseable so the delete reaches the other five objects, not to
  # reconstruct a checksum.
  export PROJECT_ID LITELLM_KSA_NAME LITELLM_GSA_NAME VERTEX_PROJECT_ID VERTEX_LOCATION
