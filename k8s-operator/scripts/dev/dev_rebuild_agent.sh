...
# The revision stamp baked into /opt/build-info.json. A dev build is usually
# dirty, and `-dirty` is the honest answer: the self-improvement runner fetches
# the source at this revision to read the code it is investigating, and a clean
# SHA on a tree with uncommitted changes would send it to a commit that does not
# contain them. Empty if this is not a git checkout, which the runner treats as
# "unknown" and refuses to investigate rather than guessing.
GIT_SHA="$(git -C "${REPO_ROOT}" describe --always --dirty --exclude '*' 2>/dev/null || echo "")"
