...
func TestCloudLoggingFilterScopesToDataAccessAndTheCluster(t *testing.T) {
	...
	c := &CloudLoggingSource{Project: "adamparco-kage", ClusterName: "gke-scratch-kube-agents-dev"}
...
func TestCloudLoggingUnavailableNamesTheRemedy(t *testing.T) {
	...
	c := &CloudLoggingSource{Project: "p", ClusterName: "c", GCloud: filepath.Join(t.TempDir(), "no-such-gcloud")}
